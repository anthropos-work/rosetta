...
>   corpus, was true at `2adcf71`.) **The production address is not stated here in either direction:** no `.tf` file in any clone names `http://backend.internal.anthropos:8081` (0 hits over all 44 tracked `.tf` files in the 13 `stack-demo` repos at each clone's own HEAD, 2026-08-06; the literal's one occurrence anywhere is a past-tense markdown KB page, `app/knowledge/service-dependencies.md:52` @ `app` `ad9f3c49`), **and** the deciding declaration lives in `infrastructure`, which has never been in any clone set. See the *RPC* line under *Interface Discovery* below, and [`platform-migration-status.md`](../architecture/platform-migration-status.md) for the fenced unmeasurable-claims convention.
