...
> | [cms](./cms.md) | cms-in-app v8.0, app **v1.360.0** (prod teardown **M810 — the ECS service is DESTROYED**, measured at `infrastructure` `13c248e6`; the legacy *schema* drop is the step still pending — see the *M810 prod teardown has now LANDED for both* bullet below — ⚠️ **this pointer called that bullet *"M810 prod teardown is UNEVEN"* until M257x iter-141, which is the title the bullet itself RETRACTED at iter-127; a cross-reference that names its target by a retracted title re-publishes the retraction, and no anchor fence can see it because the pointer still resolves**) | content layer + Directus edge + Studio — `internal/cms/` |
...
>   (`storage@4ce8ece5:cmd/root.go:62-66` — `rpcMux.Handle(storagev1connect.NewStorageServiceHandler(...))`,
>   implemented in `internal/rpcsrv/rpcsrv.go`. ⚠️ **This cited `sdk/storage/v1/service.go` until M257x
>   iter-115, and that file is the CONSUMER-side SDK, not the handler**: 47 lines declaring
>   `type Service interface {GetObject, PutObject, GetPresignedUrl}` (`:13-17`) and the dialling
>   constructors `NewService`/`NewPublicService` (`:19`, `:34`); every `StorageService` token in it is
>   `storagev1connect.StorageService**Client**`, and `git grep -n 'Handler\|ServeMux' 4ce8ece5 -- sdk/storage/v1/service.go`
>   returns **nothing**. Of three parallel citations for a sentence whose whole point is *"on their own
>   muxes"*, two landed on the construct and this one landed on its mirror image — and a line-existence
>   check can never catch it, because the file name contains the service's name), messenger's `MessengerService`
...
> * ⚠️ **RETRACTED at M258 iter-18 — there is no such edge.** Platform `766df6c` (v11.0) folded the Casbin PDP into `app` as `app/internal/sentinel/` and deleted the `sentinel` compose service; `AUTHORIZATION_ADDRESS` occurs **0** times across `docker-compose.yml`, `common.yml` and `repos.yml`, and `app` deleted its own Connect-RPC listener with it (`app/main.go:1310`, *"NO RPC SERVER"*). **A `core` stack has NO cross-process Connect-RPC edge at all.** What survives of the old qualification, and is still the point: `backend` still calls `gotenberg` over **plain HTTP** (`GOTENBERG_URL`, `docker-compose.yml:34`; `gotenberg` is in the default `core` profile, `:161`) and Judge0 via `JUDGE0_BASE_URL` (`:36`) — *no RPC edge* does **not** mean *no cross-process edge*. Canonical statement: [`platform-migration-status.md`](../architecture/platform-migration-status.md) § the `sentinel` row.
>   *(historical, at platform `0c91421`:)* **the only cross-process *Connect-RPC* edge out of `backend` on a `core` stack was `backend → sentinel`**
...
`http://backend:8083`, which is what compose set at that ref (and had set since before `d11a403`, which
re-pointed the other two of messenger's four and not this one) and sets nowhere now
...
* **External**: Clerk (auth), Stripe (payments), PostHog, Bedrock (AI), AI providers via the `ai.AI` interface — **in-tree at `app/internal/ai/` since `1e457fa70`, no longer the shared private module** (embeddings + skill matching — merged skiller domain), **Brevo** — reached in-process by both folded subsystems (transactional mail via messenger-in-app, marketing contacts via customerio-sync-in-app), each behind its own switch — and Sentry
...
# or just backend:
make up PROFILE=backend # also starts postgresql, redis, gotenberg
```
