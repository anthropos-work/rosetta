...
> `Dockerfile.dev` — which is why a subgraph SDL change rebuilt the router, as the *Build-time, static composition* bullets of this doc describe (specifically the struck-through *"`make up` rebuilds `graphql`"* bullet — ⚠️ **line-pinned, twice over, until M257x iter-138, by then the *no-hot-reload* bullet**; correct at `cd16967` (iter-102), rotted **+2**, so the bullet is named rather than re-pinned: *"It **used to** rebuild whenever any subgraph schema changed, because the build context is the parent dir (`..`) holding all sibling repos"*). **Not the *Ports* bullet**, which is about `8080`/`5050` — corrected M257x iter-102. ⚠️ **That disclaimer line-pinned the Ports bullet until M257x iter-138, by then the *Federation* bullet** (`federation_version: =2.3.2`); the Ports bullet had moved two lines down. **Correct at `cd16967`, rotted +2 — the third pin in this one paragraph to do so, which is why all of them are now named rather than numbered.**
...
* **Upstream consumers**: every GraphQL client — `next-web-app`, `studio-desk`, mobile. **They hit `backend`
  ...
  clean-up — both constructs sat at quite different lines at `0dab54d`, and different ones again at `2adcf71` — so grade the
