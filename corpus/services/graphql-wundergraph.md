...
* **Primary Goal**: Federate the platform's five Go GraphQL subgraphs into a single
  Apollo Federation v2 **supergraph**, served by a WunderGraph **Cosmo Router** at one endpoint.
* **Key Functions**:
  * Compose `app` (subgraph name `backend`), `skiller`, `jobsimulation`, `cms`, and `skillpath` into one schema.
...
schemas/                                Committed concatenated SDL (backend|cms|jobsimulation|skiller|skillpath).graphqls
...
|------------|---------------|---------|
| `Dockerfile.dev` | COPYs SDL fresh from **sibling repos** (`../app`, `../skiller`, …) and `awk`-concatenates | local `make up` (compose) |
...
|----------|------------------------------|-------|
| `backend` (the `app` service) | `http://backend:8082/graphql/query` | subgraph named `backend`, maps to repo/service `app` |
...
* **Downstream (composed subgraphs)**: `app` (as `backend`), `skiller`, `jobsimulation`, `cms`, `skillpath`.
* **Compose `depends_on`** (all `service_started`): `backend`, `skiller`, `jobsimulation`, `cms`, `skillpath`, **`storage`** — note `storage` is **not** a GraphQL subgraph but is in the startup-order list.
* **CI/prod**: GitHub Releases on `anthropos-work/{app,skiller,jobsimulation,cms,skillpath}` (schema artifacts) + `anthropos-work/infrastructure` Terraform + `release-service.yml`.
...
profile like `make up PROFILE=skiller` does **not** start the gateway — bring up the
