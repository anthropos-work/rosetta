...
> **And `sentinel` joined them at platform `766df6c`** (2026-08-11, v11.0) — the 8th fold, and the one
> that leaves `app` as the only Go service. At that ref `docker-compose.yml` declares **4** services
> (**6** effective, with `common.yml`'s `postgresql` + `redis`) and `repos.yml` carries **3** entries —
> `app`, `next-web-app`, `studio-desk`. *(This read 5 / 7 / 4 until M258 iter-18.)*
...
| [`sentinel.md`](sentinel.md) | ~~Sentinel~~ | **MERGED INTO `app` at v11.0** (`766df6c`) — still authorization only (Casbin RBAC/ABAC), but as `app/internal/sentinel/`, in-process; no container, no `repos.yml` entry, no `AUTHORIZATION_ADDRESS`. Authentication is Clerk + `colony/authn`, *not* this |
...
| [`studio-desk.md`](studio-desk.md) | Studio-Desk | **Next.js 16** design tool for authoring simulation blueprints (was TypeScript/Vite/Express until the migration) |
