...
> | **`repos.yml`** | **no `storage` entry** — `838d907` removed it, so `make init` does not clone the repo any more. Four entries remain: `app`, `sentinel`, `next-web-app`, `studio-desk`. (The entry was still there at `0dab54d`, kept then as the rollback path.) The repo itself is not deleted — clone it by hand to read the pre-merge source |
...
* **Upstream consumers**: **`app` only** — the jobsimulation domain (recordings, simulation documents),
  ...
  inside the `backend` binary. There are no `jobsimulation`/`cms` containers left to call anything — `d11a403` deleted both compose services and both `repos.yml` entries. In prod **both ECS services are destroyed**: `jobsimulation`'s at `6092c6d2` (M810 landed for that row), and `cms`'s too — measured at `infrastructure` `13c248e6`, which declares no `module "cms"` at all (`terraform/production/services.tf:64-70`; M257x iter-123). The block that survives **in cms's own repo** at `service_desired_count = 0` is **orphaned dead code**, instantiated by no root module. (This bullet called cms's prod state **UNMEASURABLE** on the ground that the deletion lands in `infrastructure`, *"never in a clone set"* — not in the standing one, which is a clone-set limit and not a measurement limit.)
