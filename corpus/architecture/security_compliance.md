...
- AI provider clients are **EU-resident by default** — Azure OpenAI EU (`ai.go:262-266`), AWS Bedrock pinned
  ...
  fallback chain (`external_services.md:546`), and the wording mattered because the two US paths *inside
  ...
  set**: [`external_services.md:570`](./external_services.md) enumerates **four live** ways a request leaves
...
- **Anthropic is reached through AWS Bedrock `eu-west-1` from the AI manager (`:85-95`) — but "Anthropic
  ...
  the `flag_use_azure_us` caveat below. [`external_services.md:542`](./external_services.md) carries the
