...
banner). ⚠️ **"The two Go repos a stack clones and builds" is now ONE — `app`.** That reading is
true at the ref it is pinned to and is kept as such; platform **`766df6c`** (v11.0, 2026-08-11)
folded `sentinel` into `app` as `app/internal/sentinel/` and deleted **both** its compose service
and its `repos.yml` entry, so at `766df6c` `repos.yml` declares **three** repos (`app`,
`next-web-app`, `studio-desk`) of which exactly **one** is `type: go`. Every "app and sentinel"
pairing in the per-library tables below carries the same expiry — each is a dated measurement at
the ref beside it, never a standing fact. The other two of the five
...
| **Version pin** | **ONE pin across the live services: `v0.35.2`.** `app` (`app/go.mod:15` @ `ad9f3c49`) and `sentinel` (`sentinel/go.mod:8` @ `f2c46190`) — the only two `type: go` entries in `repos.yml` **at platform `0c91421`** — now agree. ⚠️ **"the only two `type: go` entries in `repos.yml`" is RETRACTED at M258 iter-18 — there is only ONE.** Platform `766df6c` (v11.0) deleted `sentinel`'s `repos.yml` entry along with its compose service, so `make init` no longer clones it and `app` is the sole `type: go` entry. The `sentinel/go.mod` measurement below is still a real reading of a real repo — the repo is neither archived nor deleted — but it is **no longer a reading of anything a stack builds**, which is the whole point of a "live services" pin. On the live side the pin is now a set of one: `app` `v0.35.2`. **The long-standing "split" is CLOSED**, and the closing event is dated: `sentinel`'s `88036d7` *"chore(deps): update dependencies to latest versions"* took it `v0.34.3 → v0.35.2`, two commits past the `88bc5592` this row used to cite. The frozen repos keep their own pins and are **not** part of this reading — `storage` `v0.34.3` (`4ce8ece5`), `messenger` `v0.35.2` (`fa47850d`), archived `chronos` `v0.30.1`; the `v0.35.1` third pin went with the `cms` + `jobsimulation` husk containers at `d11a403`. Measured from each repo's `go.mod` at the ref stated beside it. |
| **Imported by** | **Every live Go service — `app` and `sentinel`, and only those two, at platform `0c91421`.** `repos.yml` there lists four entries (app, sentinel, next-web-app, studio-desk) of which two are `type: go`. ⚠️ **RETRACTED at M258 iter-18 — at `766df6c` it is `app`, and only `app`.** v11.0 folded `sentinel` into `app` (`app/internal/sentinel/`, the 8th merge) and deleted its compose service *and* its `repos.yml` entry in the one commit, so `repos.yml` now lists **three** entries (app, next-web-app, studio-desk) of which **one** is `type: go`. `sentinel` joins the frozen-legacy class below — its `go.mod` still requires colony, and nothing clones or builds it. The four-service reading (app, sentinel, **storage**, **messenger**) was true at `0dab54d` and **`838d907` ended it**: that commit deleted the `storage` and `messenger` clone entries *and* their compose services, so `make init` now clones app, sentinel, next-web-app, studio-desk. Both repos still import colony in their own `go.mod` (storage `v0.34.3` @ `4ce8ece5`, messenger `v0.35.2` @ `fa47850d`) but nothing clones or builds them — they join the `cms`, `jobsimulation` and `roadrunner` repos, **gone from compose** at `0dab54d` (`d11a403`); there is no profile that starts them and no `graphql` profile at all. Their domains run inside `app`, and the three repos are **frozen legacy** — still on GitHub as the pre-merge reference, but with no compose service, no `repos.yml` entry and nothing that starts them, so they are not importers of anything a stack runs. The `roadrunner` repo's own import was minimal but real while it lasted: `roadrunner/main.go:7` imports `colony` for `NewVersionConfig` (`roadrunner/go.mod:7` pins `v0.34.3`) |
...
| **Version pin** | **ONE value across the live services: `v1.210.0`.** `app` (`app/go.mod:16` @ `ad9f3c49`) and `sentinel` (`sentinel/go.mod:9` @ `f2c46190`) agree **at platform `0c91421`**, so **the live skew is ZERO, not two** — closed by the same `sentinel` `88036d7` dependency bump that closed the colony split (`v1.200.0 → v1.210.0`). ⚠️ **Since `766df6c` the live set is `app` alone** — v11.0 deleted `sentinel`'s `repos.yml` entry with its compose service, so its `go.mod` pin is frozen-legacy, not a live reading. A skew measured over one repo is zero by construction; the number did not change, the denominator did. At platform `0dab54d` the reading was four repos (app/messenger `v1.210.0`, sentinel `v1.200.0`, storage `v1.196.0`); `838d907` deleted the `storage` and `messenger` clone entries, so `storage v1.196.0` (`4ce8ece5`) and `messenger v1.210.0` (`fa47850d`) are frozen alongside the husks below and are not part of the live reading. The frozen repos keep their own `go.mod` and therefore their own pins (cms `v1.207.0` @ `ca50c817`, jobsimulation `v1.205.0` @ `462343b0`, roadrunner `v1.196.0` @ `87d8d44`), but **nothing clones or builds them any more**: `d11a403` deleted all three `repos.yml` entries *and* their compose services in one commit, so those pins compile nowhere. Reading them as part of the platform's skew was the error this row used to make |
| **Imported by** | every live Go service that does RPC — **`app` and `sentinel` at platform `0c91421`** (it was app, sentinel, storage, messenger at `0dab54d`; `838d907` dropped the last two from `repos.yml`). ⚠️ **At `766df6c` it is `app` alone** — v11.0 dropped `sentinel` from `repos.yml` too. Note what that does to the phrase *"that does RPC"*: `app` deleted its Connect-RPC **listener** in the same release (`app/main.go:1310`, *"NO RPC SERVER"*; `rpc.go` deleted at `a85e8308d`), so the module is imported for its generated types and hand-written domain types, not for a server this stack starts. The cms / jobsimulation / **skiller** RPC surfaces are served in-process by `app`; **skillpath and roadrunner were REMOVED, not re-hosted** — `app/main.go` registers six Connect handlers @ `app` `b948604` v1.366.0 — five unconditionally (Users `app/main.go:1187`, Organizations `app/main.go:1188`, Skiller `app/main.go:1196`, JobSimulation `app/main.go:1204`, LabSession `app/main.go:1228`) plus `CMSService` **only when a cms RPC server was built** (`app/main.go:1212-1214`, `if cmsRPCServer != nil`) — and neither `SkillPathSessionService` nor a RoadRunner service is among them |
...
[`storage.md:133`](../services/storage.md) documents in full. The named ones:
...
* [Sentinel](../services/sentinel.md) — the Casbin PDP `colony/authorization` calls. **A package, not a service, since `766df6c`** (v11.0): folded into `app` as `app/internal/sentinel/`, so the call is an ordinary in-process Go call, not an RPC to a container
