...
*   **[service_taxonomy.md](./service_taxonomy.md)**: The three-tier service categorization — core backend (Tier 1), studio + internal apps (Tier 2), and external services (Tier 3) — with each service's role, ports, and integration pattern. The "which service is what" reference. Includes the **content-vs-runtime callout**: CMS is the *content layer* (it owns skill-path and simulation *content/definitions*), while the like-named `skillpath`/`jobsimulation` services are *runtime/session engines* that reference that content by ID — `skillpath` the service ≠ skill-path content, `jobsimulation` the service ≠ simulation content.
