...
*   **[external_services.md](./external_services.md)**: The third-party integrations — Clerk (auth), Directus (the production headless CMS the platform reads content from), the AI providers, LiveKit (voice), and AWS Chime (recording) — how each is configured and consumed. It also carries the **GraphQL endpoint** section, which is now first-party: the WunderGraph Cosmo gateway was **retired 2026-07-31** and `backend` serves its own schema.
