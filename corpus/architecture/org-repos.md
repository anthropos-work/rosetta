...
| **`analytics-go`** (Go, 2025-02-12) | A two-file Brevo event tracker carrying **Stripe subscription-lifecycle events** (`app/internal/payments/handler.go:302-316`). ⚠️ **No longer a dependency** (corrected 2026-09-01): `app` `c334f559` requires no org module and the tracker is in-tree at `app/internal/tracking/brevo_tracker.go`. This cell read *"**`app/go.mod:14` `v0.3.1`** — a direct compile-time dependency"* until 2026-09-01. Untouched for ~18 months; `v0.3.1` is its newest tag. | **KEEP — DORMANT AND LOAD-BEARING. Do not delete** | [`shared_libraries.md`](shared_libraries.md) |
