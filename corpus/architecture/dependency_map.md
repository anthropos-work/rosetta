...
| **Storage** | - | **S3** only — plus a local-filesystem fallback per bucket. **No Postgres, no Redis:** storage owns no database (no `DB_CONNECTION`/`REDIS_ADDR` in compose, no redis in its `go.mod`), which is what its own doc says at [`storage.md:14,21`](../services/storage.md). Corrected M257x iter-49 |
