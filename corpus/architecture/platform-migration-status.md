...
> (local clone and origin level; `app` @ `2035f9a`, post-v1.369.0). The prior readings were 2026-08-04 at
...
2. **The teardown phase has started, and it is uneven.** The program's second half — destroying what the
   ...
   **It has already landed for `jobsimulation`** (`6092c6d2` deleted the ECS service *and* the ECR repository)
   while `cms` sits untouched at `service_desired_count = 0` and `storage`'s service block is gone by a
   different route entirely (`838d907`'s v9.0 sibling, not M810). Three folded services, three different prod
