...
| `analytics-go` | 2025-02-12 | **NOT a service — a Go library, and ⚠️ NO LONGER A DEPENDENCY OF ANY KIND** (corrected 2026-09-01). At `app` `c334f559` `go.mod` requires **no** org module; the tracker was ported byte-for-byte into `app/internal/tracking/brevo_tracker.go`. This cell said *"a DIRECT compile-time dependency of the backend monolith"* (`app/go.mod:14`, `v0.3.1`) until 2026-09-01. Two files; carries **Stripe subscription-lifecycle events → Brevo** (`app/internal/payments/handler.go:302-316`). Dormant ~18 months and load-bearing: **do not delete.** [`shared_libraries.md`](shared_libraries.md) |
...
| `hyper-studio` | 2026-08-06; **re-measured 2026-08-24** | the org's most actively developed repo (827 commits since 2026-06-17) — the successor to **studio-room**. ⚠️ **NO LONGER PRE-INTEGRATION.** *("no deployment" was correct on 2026-08-06 and went stale: the `Dockerfile` landed 2026-08-14, the compose file 2026-08-19.)* It now ships a Dockerfile, a compose file, a systemd unit and terraform. Since **v2.10 M272** its **Lodge** service is deployed in every `dev-N` and `demo-N` stack — a net-new compose block rext emits, with **no** platform-repo edit and **no** `depends_on` edge (it shares no DB, Redis, Clerk tenancy or GraphQL surface). Zero platform coupling remains the accurate half. `secrets-spec.md:370` already borrows its `.env.example`. [`../services/lodge.md`](../services/lodge.md) · [`org-repos.md` § 10](org-repos.md) |
