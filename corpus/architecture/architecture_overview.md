...
4. **Shared Libraries**: ⚠️ **ZERO imported private modules — nothing is pulled at Docker build any more.** Measured at `app` `c334f559` (2026-09-01): `grep anthropos-work/ app/go.mod` returns one line, the `module` declaration itself, and it is fenced by `app/internal/taxonomy/module_import_guard_test.go` → `TestNoFirstPartyModulesInGoMod`, which scans **both `go.mod` and `go.sum`** for any `github.com/anthropos-work/` line other than `app`'s own. **This line said *"**five** — analytics-go, colony, proto, storage, taxonomy"* (`app/go.mod:14-18` @ `ad9f3c498`) until 2026-09-01**; all five are now in-tree (`internal/tracking`, `internal/platform` + `internal/authn`, `internal/proto`, `internal/storage`, `internal/taxonomy`), and `sentinel` is not a repo a stack clones. **`ai` is NOT among them** — `app` folded it in-tree at `1e457fa70` (2026-08-04) and carries it at `app/internal/ai/`. **`authn` is not one either**: it ships inside colony as `colony/authn`, and no service's `go.mod` requires the standalone module — 0 hits for `github.com/anthropos-work/authn` across all seven Go clones, against a positive control of `colony` required by all seven. ⚠️ **This line said *"four — colony, proto, ai, taxonomy"* until M257x iter-133**, which is wrong twice: it kept `ai` after the fold and it never carried `analytics-go` or `storage`, both direct requires. The **fenced** set is [`platform-migration-status.md`](./platform-migration-status.md)'s assertion G, which prints the true module set on every run
