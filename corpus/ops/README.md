...
| [Secrets Spec](./secrets-spec.md) | Provision every repo's target `.env` (`dev-N`/`demo-N`) from one secret source (dir/zip, default `.agentspace/secrets`) — **values-blind** (no verb reads/echoes a value) — verified by the 6-repo/55-gene secret-coverage DNA + the two-tier keep-listed gate; the source-dir layout contract, alias-family vs distinct-similar rules, the waived class, and the `DIRECTUS_TOKEN` non-rearm safety. `stacksecrets` CLI, skill `/stack-secrets`. (v1.6/M27–M30) | Filling in a stack's `.env` secrets across all six repos + checking coverage |
