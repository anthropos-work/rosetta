...
| [Live Engines](./dev-live-engines.md) | Serve **lodge** (hyper-studio) and **studio-desk** natively from a git worktree against a running dev stack, so an edit is live in ~2-3 s instead of an image rebuild. The `engine-switch.sh` live/baked/stop/status switch, the `hstudio-integration` worktrees, the CORS origin the backend does not ship, and why the live panel needs no credential while the containerised one does | Developing hyper-studio and studio-desk together |
