...
| **Errors** | Sentry SDK, **production-only** | `app/main.go:279` `SentryDSN: os.Getenv("SENTRY_DSN")`; `colony/logging.go:47` early-returns unless `IsProduction()` or `FORCE_SENTRY` |
