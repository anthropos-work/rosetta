...
> | the demo env bridge carries them | `bridge_bedrock_creds` (`demo-stack/up-injected.sh:1364`) iterates a **fixed five-key list** at `:1368` — `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `AWS_SESSION_TOKEN`, `CLAUDE_CODE_USE_BEDROCK`. No Bunny key, and the function is Bedrock-specific by name and by body |
>
