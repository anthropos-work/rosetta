...
- **At ORG SCALE a single bounded re-assert is NOT enough for the heaviest data grids — use a BOUNDED
  ...
  **bounded poll** (re-settle + re-measure up to N times, return on the FIRST pass, keep polling ONLY while the
  ...
  - **Then decompose the wall — PART is demo-patchable, PART is a genuine re-scope trigger (M46).** iter-07
    ...
    **(B) NOT demo-patchable — the re-scope trigger.** `/enterprise/members`' per-row `targetRole` →
    `OrgCheckActionPermission` Sentinel RPC checks `OrgActionAssignmentsWrite`, which is **PER-OBJECT** (per
    assignee) — a manager can-write-assignments for some members and not others. A cache/singleflight keyed by
    `(org, subject, action)` (dropping the object so it dedupes across rows) is a **CORRECTNESS BUG**: it returns
    the first row's allow/deny for every row → `failed to get target role: forbidden` on legitimately-allowed
    members (~1744×/sweep) → the grid errors, NOT just slows. Keyed correctly `(org, subject, OBJECT, action)` it
    can't dedupe (every row a different object). The per-row Sentinel fan-out can ONLY be collapsed by a
    **DataLoader / batch `BulkCheckPermission` RPC = a PLATFORM change** (forbidden). So `/enterprise/members`
    **at org scale is the genuine re-scope trigger** — surface it; do NOT fake the gate with a permission-poisoning
    cache. **Lesson: decompose a perf wall before judging it — an over-broad fetch + a missing index are
    demo-patches, but a per-OBJECT authz RPC is platform-bound; "wrap it in the existing cache like the
    org-feature precedent" is a TRAP when the underlying permission is object-scoped, not org-scoped.**
