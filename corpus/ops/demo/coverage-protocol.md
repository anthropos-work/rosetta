...
>   client calls `https://<magicdns>:<8082+offset>/graphql/query` — `backend`'s own endpoint. (At M219 this
>   read `<5050+offset>/graphql`, the Cosmo router; the router was **retired 2026-07-31** and nothing binds
>   `5050+offset` any more. The failure mode below is unchanged — only the port is.)
...
>
> Measured on `billion` at M219, when the router still ran: from the host, https on `:13000`, `:15050` **and**
> `:18082` all fail TLS; **from a tailnet peer all three answer.** (Read that as `:13000` and `:18082` today —
> `:15050` no longer exists.) The demo was healthy throughout. The first M219 sweep run this way reported
...
- **At ORG SCALE a single bounded re-assert is NOT enough for the heaviest data grids — use a BOUNDED
  ...
  **bounded poll** (re-settle + re-measure up to N times, return on the FIRST pass, keep polling ONLY while the
  ...
  - **A poll that EXHAUSTS on a never-resolving query is a PERF wall — diagnose N+1-vs-combination before
    ...
    GetOrganizationTargetRole`, no DataLoader), and the GraphQL layer logged **10–84 s** latencies — the page sits
    on a `…` spinner / skeleton through the poll. Tell it apart from a slow-paint by **measuring the query, not
    the pixels**: (a) grep the GraphQL request log for 10 s+ requests; (b) the raw SQL for the same shape is
    **milliseconds** — so the wall is the **RPC round-trip COUNT**, not the DB.

    > **The log to grep moved.** The M46 measurement read `docker logs <stack>-graphql-1` — the Cosmo router,
    > **retired 2026-07-31**. There is no `<stack>-graphql-1` container; that command now prints nothing and
    > exits non-zero, which reads like "no slow queries" rather than "wrong container". GraphQL is served by
    > `backend`, so grep **`docker logs <stack>-backend-1`**. The queries described here were never the
    > router's own work — it only forwarded them — so removing the hop does not remove this wall. The
    > per-row resolver fan-out and the per-membership Sentinel RPC are unchanged.
