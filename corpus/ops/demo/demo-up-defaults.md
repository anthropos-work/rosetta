...
| `DEMO_ADVANCE_CLONES` | `0` | **OFF — no auto-advance** (deliberate staleness at a known-good rev is legitimate; an unconditional pull could break a demo mid-presentation). `=main`/`=1` brings every clone to `origin/main` (the platform `make pull` primitive); `=pinned` checks each out at its `stack-demo/clones.pin.json` ref | `ensure-clones.sh:220` |
| `DEMO_FRESHNESS_STRICT` | `0` | **advisory — a stale/pin-drift/fetch-failed clone WARNS loud but does not block** (the build-SOURCE gate is elsewhere). `=1` escalates any not-provably-fresh clone to a FATAL abort (for a CI / HARD go/no-go bring-up that must build current code) | `ensure-clones.sh:475` |
