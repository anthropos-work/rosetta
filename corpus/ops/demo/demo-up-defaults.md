...
|---|---|---|
| **`demo-stack/up-injected.sh`** — the **full bring-up**; what `/demo-up N` actually runs | `<N>` (positional), **`--public-host <host>`** and **`--no-public-host`** — *that is the entire flag surface* | **hard-errors:** `unknown argument` → **`exit 1`** (`up-injected.sh:26-27`) |
...
> ⚠️ **That cell omitted `--no-public-host` until the M258 close**, while the CLI table 140 lines below
> carried its row all along — so the file contradicted itself about its own entry point, and the guard
> stayed green because it reads table rows, not this prose. It matters more since M258 than it did before:
> **`--no-public-host` is the flag that decides whether the batch gate runs or skips**, so a reader who
> trusted this cell would conclude the one mode in which a bring-up gates itself does not exist.
...
|---|---|---|---|
| `DEMO_STORIES` | `1` | **Stories & Heroes seed is ON.** Seeds the **4-org** roster — Cervato Systems / Solvantis / Northwind Aviation, each with a thriving/struggling/manager hero trio, **plus Meridian Talent**, the v2.4 M223 HIRING org (recruiter vantage, `is_hiring: true`) | `up-injected.sh:276` |
| `DEMO_NO_STORIES` | `0` | (opt-out of the above) `=1` restores the legacy structural **`small-200`** single-identity seed | `up-injected.sh:277` |
| `DEMO_STORIES_PRESET` | `stack-seeding/presets/stories.seed.yaml` | the preset the stories seed reads | `up-injected.sh:279` |
| `DEMO_NO_SETDRESS` | `0` | **set-dress is ON** — cache-first snapshot replay (taxonomy + directus + sim-embeddings) then the seed | `up-injected.sh:251` |
| `DEMO_NO_LOCAL_CONTENT` | `0` | **per-stack Directus is ON** — content is self-contained. `=1` reads content **live from prod** (the documented fallback) | `up-injected.sh:257` |
| `DEMO_NO_DIRECTUS_DRIFT_FIX` | `0` | the local-content Directus drift fix runs (only when local content is on) | `up-injected.sh:2885` |
| `DEMO_NO_CONTENT_URL_REWRITE` | `0` | the `demo_web` content-URL rewrite runs (only when local content is on) | `up-injected.sh:2922` |

### The UI tier & the presenter cockpit

| Knob | Default | Effect at default | Read at |
|---|---|---|---|
| `DEMO_NO_UI` | `0` | **full UI tier is ON** — next-web + studio-desk (containers) + ant-academy (native) | `up-injected.sh:245` |
...
| `DEMO_NO_COCKPIT` | `0` | **presenter cockpit is SERVED** (only when `DEMO_STORIES=1`) on `7700 + N·10000`. ⚠️ **A password-free "become any seeded hero" launcher** — see [`../safety.md`](../safety.md) **§3.2** | `up-injected.sh:2947` |
| `DEMO_NO_DIRECTUS_ENDPOINT_FIX` | `0` | **import file uploads go to THIS stack's Directus** — applies the `app-directus-upload-endpoint` demo-patch to the build-scratch app clone. Upstream's `PostMultipart` builds the upload endpoint from a LITERAL `https://content.anthropos.work/files` and ignores the configured `c.baseURL`, so every file a simulation import writes (source zip **unconditionally**, avatars, assets) is addressed at **production**. `=1` builds without it and those uploads target prod — they 401 on a demo, whose `DIRECTUS_TOKEN` is emptied, so this knob buys **correctness, not safety**; the exposed side is a **dev** stack holding a real token. **FATAL on a broken anchor.** See [`../safety.md`](../safety.md) §2.3 | `up-injected.sh:2149` |
| `DEMO_NO_LODGE` | `0` | **the LODGE service is DEPLOYED** (hyper-studio) — the job **wire** on `8080 + N·10000` and the **operator panel** on `7787 + N·10000`, both world-published like every other demo port. `=1` brings the demo up with no lodge at all. The panel is gated by a shared secret sent as HTTP Basic (any username, the secret as password) — see [`../safety.md`](../safety.md) **§3.2** and [`../../services/lodge.md`](../../services/lodge.md) | `up-injected.sh:2301` |
...
| Knob | Default | Effect at default | Read at |
|---|---|---|---|
| `DEMO_NO_PATCH` | `0` | **all demo-patches applied** | `up-injected.sh:584` |
| `DEMO_ALLOW_UNBAKED_PATCH` | `0` | a demo-patch whose baked marker is absent is a **refusal** (fail-closed). `=1` lets the bring-up proceed with the patch unapplied. ⚠️ Undocumented until v2.10 M272 — surfaced by `demo_knob_guard` while adding `DEMO_NO_LODGE`, not by this milestone's own work | `up-injected.sh:1044` |
| `DEMO_NO_AUTHZ_SKIP` | `0` | ⚠️ **the `app-targetrole-authz-skip` patch is APPLIED** — authorization is short-circuited on the per-member target-role write path. Part of what makes a demo an **authz-weakened build** ([`../safety.md`](../safety.md) §3.2) | `up-injected.sh:1900` |
| `DEMO_NO_AIREADINESS_LOADMEMBERS_BOUND` | `0` | the `app-aireadiness-snapshot-loadmembers` read-path patch is applied | `up-injected.sh:1901` |
| `DEMO_NO_PERF_INDEXES` | `0` | the demo perf indexes are created | `up-injected.sh:2844` |
| `DEMO_NO_SENTINEL_RELOAD` | `0` | the casbin-policy invalidation runs after seeding (the silent-403 catcher). **Since platform `766df6c` there is no `sentinel` container to reload** — the knob now gates a Redis Pub/Sub publish to `sentinel:policy:invalidate`, which the in-process PDP subscribes to | `up-injected.sh:2802` |
...
| `DEMO_NO_MKCERT` | `0` | the local-trust cert is minted (the localhost path) | `up-injected.sh:263` |
...
| `DEMO_NO_SECRET_PREFLIGHT` | `0` | the secret-coverage pre-flight runs (a CRITICAL miss is fatal) | `up-injected.sh:1768` |
...
| `DEMO_REUSE_DEV_IMAGES` | `0` | **OFF — full independence.** The demo builds from its **own** `stack-demo/` clones; dev's images are never reused | `up-injected.sh:2283` |

### Host pre-flight & verification

| Knob | Default | Effect at default | Read at |
|---|---|---|---|
| `DEMO_NO_HOST_PREFLIGHT` | `0` | the host pre-flight runs (non-fatal — warns, never blocks) | `up-injected.sh:446` |
| `DEMO_ALLOW_MISSING_HOST_MOUNTS` | `0` | **the host bind-mount pre-flight runs, and it is FATAL** (v2.8 M257x iter-56 — the only fatal member of this section). Every host-absolute bind source the default profile's services declare must exist **and not be an empty directory**: `docker-compose.yml` mounts `$HOME/.aws/credentials`, and when that path is absent Docker neither fails nor warns — it **creates the source as an empty DIRECTORY** and mounts it, after which `backend` dies on `is a directory` and **exits 0 in 137 ms**, which every log-reader and supervisor reads as an orderly shutdown. Measured three ways on one image: no mount → starts; the auto-created directory → the exact 2-line dead signature; a regular empty file → starts and stays up. The set is **derived** from the compose file, not written down (`stack-injection/platform_topology.py check-host-mounts`). `=1` proceeds anyway — for the operator who knows the mount is genuinely unneeded | `up-injected.sh:2535` |
| `DEMO_VM_MIN_GIB` | `12` | Docker-VM RAM floor asserted by the pre-flight | `up-injected.sh:316` |
| `DEMO_DISK_MIN_GIB` | `25` | free-disk floor asserted by the pre-flight. **Re-sized from `20` at v2.8 M255** — the old value was reasoned from a "~3.7 GB frontend build / ~3.7 GB build cache" that is stale by an order of magnitude. It is now **derived from measurement**: one cold-images cycle peaks at **18 GiB** of consumption and leaves ~2 GiB behind, + a 7 GiB reserve. Same arithmetic as `stack-core/hostprofiles/billion.json` (`projected_image_gib` 18 + `disk_floor_gib` 7), fenced by a test. Still **non-fatal** — the hard-failing twin lives in `buildbench` (see [`build-budget.md`](build-budget.md)) | `up-injected.sh:403` |
| `DEMO_CERT_RENEW_DAYS` | `30` | the FAPI TLS cert is **re-minted when it expires inside this window** (also when absent, or minted for a different host). **Net-new at v2.8 M255:** `$STACK/certs` survives `--purge` and the mint used to be guarded on file-absence alone, so the first bring-up on a host was the only one that ever minted — a 90-day Let's Encrypt cert then silently expired and only a *remote* browser noticed. See [`../safety.md`](../safety.md) §3.5.4 | `up-injected.sh:1664` |
| `DEMO_NO_VERIFY` | `0` | **the bring-up auto-verify runs** on the stack's own offset ports (non-fatal). See [`../verification.md`](../verification.md) | `up-injected.sh:3111` |
| `DEMO_NO_BATCH` | `0` | **the bring-up drives the full Playthrough batch** at its tail and leaves the stack in a presenter-usable world (v2.8 M258). The layer above `DEMO_NO_VERIFY`: autoverify proves the stack is *reachable and healthy*, this proves it is *functionally correct* — so "UP" means **"UP, and every journey verified"**. Under **`D-v28-3`**: runs to completion, **never** halts at first red, **never** retries, **ONE** consolidated red set at batch end, the stack left **UP regardless**, and the bring-up exiting **non-zero and loudly** on a non-empty set. ⚠️ **It SKIPS itself on a `--public-host` stack** — such a demo cannot be browsed from its own host (docker-proxy binds `0.0.0.0`, bypassing `tailscale serve`), so running it there would red every Playthrough for a reason unrelated to the product. The skip records `verdict: skipped`, which is **never** `green`; the peer path (`--reset-only` here, browser half from a tailnet peer) is printed. Since `--public-host` is default-on, **a bare `/demo-up N` skips; `/demo-up N --no-public-host` gates.** ⚠️ **It also skips on any knob that removes what it needs** (added at the M258 close, the same rule generalised): `DEMO_NO_STORIES=1` / `DEMO_STORIES=0` takes away the heroes, the cockpit AND the per-stack `bin/stackseed`, and `DEMO_NO_UI=1` takes away every browser surface. Both used to run anyway — the first reporting **ERROR rc=3 on a fine bring-up**, or worse, where an earlier stories run had left `bin/` behind (it survives `--purge`), **TRUNCATing the operator's small-200 world and re-seeding stories over it**; the second reding all 30. Verdict artifact: `$STACK/batch-gate.json` — written on **every** path including the skips, because `--purge` does not clear the stack dir and "no file" would otherwise be indistinguishable from the previous run's verdict. See [`../verification.md`](../verification.md) + [`playthroughs.md`](playthroughs.md) | `batch-gate.sh:149` |
