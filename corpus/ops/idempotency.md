...
| **lodge** (v2.10 M272) | `stack-core/lib/lodge.sh` — `lodge_acquire` · `lodge_build` · `lodge_env_write` | **SAFE** (idempotent), with one ⚠️ | The **clone** is reused if present (never re-cloned, never clobbered — a hand-placed tree is taken as-is). The **image build** is keyed on the checkout's short sha and short-circuits on `docker image inspect`, so a second stack on the same commit pays nothing. The **env profile** is rewritten from scratch each bring-up, which is what keeps the panel's `LODGE_PANEL_AUTHORITIES` in step with the stack's own offset port. ⚠️ **The DATA is not preserved across a teardown**: `lodge-data` is a named volume and *both* teardown paths pass `down -v` — even without `--purge` — so `dev-down`/`demo-down` destroy that stack's whole lodge job store, forgespace and archive. Re-running `up` on a LIVE stack is refused by the registry, so the only way to re-run is to tear down first, i.e. the data loss is unavoidable by construction. See [`../services/lodge.md`](../services/lodge.md). |
