...
3. **Build the tool** from a **pinned-tag** `rosetta-extensions` clone (the standard two-clone policy — the
   ...
   # M217: read the tag from the SoT (.agentspace/rext.tag) — NEVER hardcode one here.
   # This line used to pin `stage-door-m30` (a v1.6-era tag) and check it out in THE SAME CLONE /demo-up
   # pins — and /demo-up INVOKES /stack-secrets. So every bring-up silently dragged the consumption clone
   # BACK to a v1.6 tag: an ACTIVE drift injector that made re-pinning a no-op within a single run.
