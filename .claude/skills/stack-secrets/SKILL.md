...
3. **Build the tool** from a **pinned-tag** `rosetta-extensions` clone (the standard two-clone policy — the
   ...
   demo-N), checked out at the latest tag **`stage-door-m28`** (which carries `provision` + the demo-aware
   `check`):
   ```bash
   SECDIR=stack-demo/rosetta-extensions   # or stack-dev/... for a dev-N
   git -C "$SECDIR" fetch --tags --quiet && git -C "$SECDIR" checkout --quiet stage-door-m28
...
- Tool tag: `stage-door-m28` (the latest; carries `provision` + demo-aware `check`).
