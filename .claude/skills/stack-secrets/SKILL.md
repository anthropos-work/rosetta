...
> `/stack-secrets --provision` step. `DEMO_NO_PROVISION=1` opts out (the legacy `stack-dev/platform/.env`
> base). Use this skill standalone to provision a **dev** stack, to provision up front, or to `check`/inspect.
