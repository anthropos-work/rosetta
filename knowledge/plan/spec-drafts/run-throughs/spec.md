# Run-throughs — Functional-Flow E2E Testing Spec

> **Status:** Consolidated draft `v0.1` · spec-draft · 2026-06-28
...
> *Proposed brand: **"Run-throughs"** — the name is the one open naming decision (see spec-progress #1).*
...
A **Run-through** is an automated actor that *is the user*. It logs in as a seeded hero, sets out with a goal,
and performs a real journey through the platform — start to finish, the way a person would — then proves the
...
identical."* We ship and refactor freely; a Run-through breaks **only when a capability breaks** — never when a
...
world; the M42 coverage sweep proves every page a hero reaches **shows** real content. Run-throughs prove the
...
- **prove** the platform's functional flows work end-to-end, performed as a human,
...
Run-throughs deliberately are **not** — are in §7 + [`next-release.md`](next-release.md).)

---

## 2. The model & vocabulary

Run-throughs are declared in a **manifest** with a four-level hierarchy — deliberately mirroring the seeding
...
└─ Story            (2) an interconnected flow of product use — a coherent journey (may span products)
   └─ Use Case      (3) one GOAL + the platform FLOW that serves it + the INTERMEDIATE & FINAL expectations
      └─ Run-through (4) the deterministic e2e test that PERFORMS the use case as a human and ASSERTS its expectations
```
...
- **Story** — an *interconnected flow of product use*: a coherent journey a real user takes, possibly spanning
  several products. The same notion of "story" that seeding uses; where it helps, a Run-through story **reuses a
...
- **Run-through** — the deterministic, automated e2e test that **performs one use case** as a human and asserts
  its expectations. **One use case ↔ one Run-through**, traceable both ways by a stable id.
...
that must keep passing). Coverage is simply: *use cases with a passing Run-through ÷ declared use cases.*

**Symmetry with seeding.** Seeding writes the world; Run-throughs exercise it. Wherever possible the **same
seeded stories & heroes** are the substrate the Run-throughs act on — the seeded world is the stage, the heroes
are the cast, the use cases are the script.
...
> new Run-through that violates a principle is wrong even if it passes. Every Run-through and every reviewer
> holds to all of them.

- **P1 — Be the human.** Drive the **real UI** as a user would. The action *under test* uses no API / DB / admin
  backdoor — if a user does it by clicking, the Run-through clicks. Verify **user-observable outcomes**.
...
- **P2 — Functional truth, not pixel truth (the cardinal rule).** Assert on the **goal achieved** —
  ...
  slight UI or wording change **must not** break a Run-through. If a Run-through fails, a *capability* failed.

- **P3 — Implementation-agnostic, zero platform coupling.** Rosetta's hard rule is **zero platform-repo edits** —
  so Run-throughs **cannot** rely on test hooks added to the platform (no bespoke `data-testid` contract). They
...
- **P4 — One use case ↔ one Run-through.** Each test proves **exactly one** declared use case, is **isolated**
...
- **P5 — Manifest-first.** The **use case is declared first** — goal + flow + expectations — independently of its
  test. The Run-through *implements the declaration*. The manifest can list a use case **before** its Run-through
  exists (a known gap), which is exactly what makes it a **build reference** as well as a regression one.

- **P6 — Deterministic, repeatable, seeded.** A Run-through binds to a **known stack state** (a seeded demo/dev
  world). Same inputs → same result, every time. No flakiness, no time/order/network coupling that isn't
  controlled. A flaky Run-through is a defect in the Run-through.
...
- **P7 — Stories compose; use cases prove independently.** A story is an *interconnected* flow, so its use cases
  ...
  verifiable** from a declared seed, so a single Run-through can run and prove its one truth.
...
A use case is the unit of functional truth. It declares (and its Run-through asserts) the following — note that
every expectation is written at the **outcome / semantic** level, per **P2**:

| Field | Meaning |
|---|---|
| `id` | Stable identifier; the 1:1 link to its Run-through. |
...
| `runthrough` | The id of the test that proves it (may be `TODO` while it's still a build-reference gap). |
      ...
      - id: hiring.shortlist-flow
        ...
        use_cases:
          - id: hiring.shortlist-flow.UC1
            ...
            runthrough: rt-hiring-shortlist-uc1
```
...
Run-throughs **extend the existing M42 Playwright foundation** rather than introducing a parallel stack.
...
fields of §4. It is authored and version-controlled as the build+regression source of truth; the Run-through
...
Run-throughs run against a **seeded stack** (`demo-N` / `dev-N`) at a **known state**, and **log in via
Clerkenstein** as the seeded hero the use case names — reusing the **stories & heroes** seeding produces (the
seeding ↔ Run-through symmetry of §2). Deterministic seed → deterministic Run-through (P6). The binding between
a manifest seed and the actual seeded world is an open item (spec-progress #5).

### 5.5 Mapping, traceability & reporting

Every Run-through is **tagged with its use-case id**; a report reconciles the manifest against results into a
**three-state map** per use case — `passing` / `failing` / `unimplemented` (a declared use case with no
Run-through yet). That map **is** the coverage dashboard and the regression reference: it answers both *"what
...
dedicated `run-throughs` section if it grows to warrant one. Authored + **tagged** like all rext tooling and
...
|---|---|
| **Seeding (stories & heroes)** | The **substrate** + shared vocabulary. Run-throughs act on the seeded world; reuse its heroes & stories. |
| **M42 semantic-coverage sweep** | **Complementary.** Coverage proves *presence* (every reachable page shows real, believable content); Run-throughs prove *function* (the hero can complete the use case end-to-end). Presence vs. function; Run-throughs are the deeper, goal-level guarantee. |
| **Clerkenstein** | The **actor's identity** — Run-throughs log in as a seeded hero through it. |
| **Demo / dev stacks** | The **target environment** the suite runs against. |
| **Alignment tests** (mirror-engine fidelity) | A **different axis** — alignment measures how faithfully a mirror reproduces a source engine; Run-throughs measure functional truth of the real platform. |

---

## 7. Out of scope for this capability (non-goals)

Run-throughs are **functional-flow truth** and nothing else. The following are different test classes,
...
- **Performance / load / stress testing** — Run-throughs assert *function*, not latency/throughput.
- **Unit / integration testing** — a platform-repo concern, below the user-facing flow.
- **API-contract testing** — Run-throughs verify at the user surface, not the wire.
...
Tracked, worked one at a time, in [`spec-progress.md`](spec-progress.md). Headlines:

1. **The brand name** — proposed *Run-throughs*; alternatives *Playthroughs / Encore / Odysseys*.
2. **Pure-semantic vs. a "semantic landmark" convention** (under zero-platform-edit) — §5.2.
3. **The authoritative manifest schema** + file layout + validation — §5.3.
4. **Stack binding** — a dedicated Run-through seeded world vs. the existing demo stories world — §5.4.
5. **Harness relationship** — extend the M42 `stack-verify/e2e` harness vs. a separate `run-throughs` section — §5.6.
6. **First coverage targets** — which products/stories to declare first (the build backlog — *after* this spec).
