...
`v1.7`; records archived under [`releases/archive/`](releases/archive/)). **Rosetta is now between releases** — v1.7
"house lights" (the **demo-UI-hardening release**) shipped 2026-06-15 (tag `v1.7`, merged `--no-ff` → main); the **next
version is unplanned** (run `/developer-kit:design-roadmap` to scope it). v1.7 made a fresh browser at a demo's offset UI
render the working app with **zero manual steps**: triggered by a live defect (next-web at `http://localhost:33000`
blanked because clerk-js's handshake to the fake FAPI hit an untrusted self-signed cert), it shipped 2 strictly-sequential
`section` milestones M31→M32 — **M31** automated a locally-trusted **mkcert** FAPI cert into the demo bring-up (openssl
fallback + `DEMO_NO_MKCERT` opt-out; fake BAPI is plain HTTP → out of scope) and **M32** fixed the sibling studio-desk
...
**Active:** **between releases — v1.7 "house lights" SHIPPED 2026-06-15** (tag `v1.7`, `release/01.70-house-lights` merged
`--no-ff` → `main`). The **demo-UI-hardening release** delivered 2 strictly-sequential `section` milestones M31→M32:
**M31** mkcert-trusted FAPI cert (a locally-trusted `mkcert` cert minted into the demo bring-up so a fresh browser at
next-web renders instead of blanking; openssl fallback + `DEMO_NO_MKCERT` opt-out; fake BAPI plain HTTP → out of scope)
→ **M32** studio-desk single-port/production (a 1-line `NODE_ENV=production` override fix so studio-desk stops 302-ing to
the dead `:9100`, + the `:9100` doc/CORS sweep). ant-academy demo liveness stayed backlog (M33, repro-first). The **next
version is unplanned** — run **`/developer-kit:design-roadmap`** to scope it. v1.6 "stage door" SHIPPED 2026-06-14 (tag
`v1.6`) was the prior release. _(Live state: [`state.md`](state.md). Backlog: [`roadmap-vision.md`](roadmap-vision.md).)_
