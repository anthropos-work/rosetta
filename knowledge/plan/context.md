...
M26→M29 (DNA+ingest → engine+gate → docs+skill → field-bake), all `section`-shaped, strictly sequential. **Tooling + docs
...
The **secret-provisioning release** — 4 strictly-sequential `section` milestones M26→M29: **M26** secret-coverage DNA +
source ingestion (the new `stack-secrets` section; ingest dir/zip; the secret-DNA `introspect`+`diff` "keep-listed" gate)
→ **M27** provisioning engine + coverage/verify gate (`provision` writes per-repo `.env`, idempotent + N=0-guarded,
composes-with-the-injection-override; `check`/`measure` wired non-fatally into `/dev-up`+`/demo-up` pre-flight) → **M28**
docs + `/stack-secrets` skill + corpus wiring (author `corpus/ops/secrets-spec.md`; retire the manual-copy prose) → **M29**
