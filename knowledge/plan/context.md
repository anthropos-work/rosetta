...
**Active:** **v1.10 "method acting" — IN DEVELOPMENT** (designed 2026-06-24 via `/developer-kit:design-roadmap`;
branch `release/01.10-method-acting` cut from `main`). The **believable-profile release** — make each hero hold up
under a close-up when a presenter *Logs in as* them: profile identity (org name + role + real-face avatar) + the
content-surface unblock (library + activity feed via one per-stack-Directus serve-grant) + profile depth
(work/education/deep role-aligned skills) + **100% per-vantage demo coverage** proven by a **Playwright** sweep (DOM
+ screenshots; zero empty pages, zero out-of-demo escapes). 5 milestones **M39→M42m**: { M39 profile identity ∥ M40
Directus serve-grant } → M41 profile depth → **M42e** employee coverage → **M42m** manager coverage (M42e/M42m
`iterative`; the rest `section`). Tooling + docs only — zero platform-repo edits. Designed from the live-demo review
[`.agentspace/profile_gaps.md`](../../.agentspace/profile_gaps.md) + the root-cause workflow `w7t4wq2z4`; milestone
records under [`releases/01.10-method-acting/`](releases/01.10-method-acting/). **Last shipped:** **v1.9
...
**`/developer-kit:build-milestone`** (M40 ∥ M39). _(Live state: [`state.md`](state.md). Backlog:
