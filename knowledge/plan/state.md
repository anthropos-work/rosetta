...
phase: "M257x iter loop, **129 iters** (+ 26 harden passes). **Gate 4 of 5**; clause 5 open, `P` UNMEASURED. **Headline: the census reads 39.7 % of the corpus's consequence surface — `corpus/ops/**` + `CLAUDE.md` are OUTSIDE it, 724 unevidenced consequence assertions, and iter-129 read all 724.** It found the MILESTONE'S OWN drift class there: 39 stale `jobsimulation.*`/`skillpath.*`/`local_*` tokens in 8 ops docs, two of them RUNNABLE (a psql block that 42P01s, a bring-up probe floor on a dropped relation). **Keep the two accountings apart — clause 5 is NOT re-cut and none of this is added to it (`F4`).** iter-128's `/api/invitations` alarm CORRECTED: token-authenticated, deliberately pre-login. R3 re-audited on its own population: **93.3 %**, not the imported 100 % → complement `cite` 90.0 → **86.1 %**. Detail: `iter-129/progress.md`."
