...
active_milestone: "M257x — platform re-alignment (iterative) — IN PROGRESS, 115 iters + 25 harden passes closed (branch m257x/platform-realignment). Re-align BOTH rosetta (corpus) and rosetta-extensions (tooling) to platform @ origin HEAD. Gate 4 of 5; CLAUSE 5 — met only by a reading that returns zero — is the only open one. M257 PAUSED behind it after 3 iters."
last_closed: "M256 — 2026-07-30"
phase: "M257x ITER LOOP, local to the new Mac (D-v28-15). **Gate 4 of 5.** Clauses 1–4 hold; **CLAUSE 5 is the only open one**, met ONLY by a reading that returns zero — ruled FOUR times, never re-cut or narrowed. **`TOK-07` (iter-110, deliberate + non-terminating) revised the strategy on iter-109's read (`P = 24` / `N = 36`, series by predicate 22 → 22 → 24, THE POOL DOES NOT DRAIN): the repair's DENOMINATOR moves from a prior reading's DETECTIONS to the corpus, per predicate.** `TOK-06`'s premise is refuted (frozen clones still yield ~33 % drift ⇒ iter-103's 61 % measured COMPOSITION, not FLOW) but its induction fences WORKED (21 % → 5.6 %) and are **re-ranked, not reverted**. Order: enumerate → repair whole predicates → **read last**; a `P ≥ 15` after a full cycle is a PRE-REGISTERED refutation of repair-and-read, not an eighth revision. **Steps 1 and 2 are COMPLETE.** iter-111 landed step 0 (suite-hang claim REFUTED — `1 failed · 1011 passed` / 1090.88 s, invocation stated); iter-112/113 landed the enumerator (`FENCE-M257x-iter112`) and its ceiling — the 7.28x DID NOT SURVIVE the form review and settled at **29 seeds → 71 sites, 2.45x**, with 16 NO-EXPANSION predicates split 1 `SMALL-CLASS-PROVEN` / 15 `SMALL-CLASS-ADJUDICATED` (254 ceiling candidates excluded with named reasons — **one agent's readings, audited by `FIX-M257x-iter113-adjudication-is-judgement`, now carrying 3 MEASURED misses from iter-115**); iter-114 made the reach metric name its denominator or print no percentage. **iter-115 landed the repair: 71/71 sites, and 24 of 24 predicates closed at EVERY enumerated instance**, both pair-halves with their twins, 7 incremental commits, baseline `0/71` reproduced first. **Step 3 — THE READ — is iter-116's entire content and is unblocked.** No point estimate of the pool is quoted — Chapman retired; floors only (≥ 36 at `ac48e5b`). **`P` is UNMEASURED since iter-109, not unmoved.** Detail: milestone `progress.md` + `decisions.md`. POINTER field per `context.md` § state.md contract; do not grow it back."
