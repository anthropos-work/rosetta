...
active_milestone: "M257x — platform re-alignment (iterative) — IN PROGRESS, 117 iters + 25 harden passes closed (branch m257x/platform-realignment). Re-align BOTH rosetta (corpus) and rosetta-extensions (tooling) to platform @ origin HEAD. Gate 4 of 5; clause 5 the only one open. Under TOK-08 — the USER's re-scope. M257 PAUSED behind it after 3 iters."
last_closed: "M256 — 2026-07-30"
phase: "M257x ITER LOOP under **TOK-08 — the USER's re-scope of 2026-08-07**, recorded in milestone `decisions.md`; NOT agent-authored. **Gate 4 of 5.** Clauses 1–4 hold; **clause 5 is the only open one and is met ONLY by a reading that returns zero** — four user rulings, never re-cut. `TOK-07` was falsified by its own sealed rule at iter-116 (`P = 37` against `P >= 15`). **`TOK-08` replaces SAMPLING with a CENSUS** over the mechanically-decidable classes, largest first, and pre-registers its own arithmetic falsification: after one full mechanical sweep, a reading at **`P >= 19`** refutes enumeration-first and hands the milestone back for a user scope decision; **`P <= 18`** means it is working. Chapman retired — floors only (**>= 37 at `f581de09`**). Detail: milestone `progress.md` + `decisions.md` § TOK-08. POINTER field per `context.md` § state.md contract; do not grow it back."
