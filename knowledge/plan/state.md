...
active_milestone: "M257x — platform re-alignment (iterative), **256 iters** + 62 harden passes closed (branch m257x/platform-realignment). Re-align BOTH rosetta (corpus) and rosetta-extensions (tooling) to platform @ origin HEAD. **The USER set a binding closing condition 2026-08-10** (`D-M257x-256-1`) that supersedes gate-4-of-5 as done: CURRENT `main` must assemble into a WORKING STACK, **demo AND dev**, with deprecated repos no longer treated as live. M257 PAUSED behind it."
phase: "M257x iter loop, **256 iters** (+ 62 harden passes; 8 tiks unhardened). Clause 5 last read **`P = 29` / `N = 47`** (iter-131) and UNMEASURED since — a FLOOR over a suspicion-selected sample, never a corpus error rate. **iter-256 advanced `app`/`next-web-app`/`ant-academy` AND `demo-stack/clones.pin.json` to origin/main; NOTHING is proven to BUILD there** — clause 1 + the dev half need a quiet host. It also refuted its own repair: the corpus is a **TWO-CLOCK document** (`anchor_construct_guard` reads `origin/main` first), so a ref-delta is **not** a repair instruction; 27 blind renumberings were applied and reverted byte-for-byte (`§5` rule 79). **Detail lives in the per-iter `progress.md` files and the milestone `progress.md`. This field is a POINTER (`context.md` § state.md contract); do not grow it back.**"
