...
active_milestone: "M257x — platform re-alignment (iterative) — IN PROGRESS, 14 iters closed (branch m257x/platform-realignment). Find where the microservices→app consolidation actually stands, then make BOTH rosetta (corpus) and rosetta-extensions (tooling) work against platform @ origin HEAD. M257 is PAUSED behind it after 3 iters."
last_closed: "M256 — 2026-07-30"
phase: "M257x ITER LOOP on the new Mac (D-v28-15 machine move DONE; odysseus retired, billion demo-only). Gate 5 clauses: (4) CLAIMABLE — REXT_TRANSITIONAL_SCHEMAS is EMPTY, coverage machine-checked (iters 06-08); (1) BLOCKED at 0/3 — iter-12 measured that platform origin HEAD `2adcf71` (2026-07-31 15:58, mid-milestone) **deleted the WunderGraph/Cosmo router outright** (service + repos.yml entry + clone; GraphQL now straight from `backend` at :8082/graphql/query, path changed too), so `docker compose config` rejects the demo project — *"hiring-app depends on undefined service graphql"* — and a cold cycle cannot start at all. **iter-13 re-pointed rext off it** (~14 files, not the 6 routed: the endpoint is DERIVED once now, not written out six times) and landed FENCE-M257x-iter13-compose-service-exists, watched RED — `docker compose config` is RC=0 again. **iter-14 then MET clause 1**: three consecutive cold `demo-down --purge` + `demo-up` cycles at `warnings:0/green:true` (11:43:02Z / 11:53:04Z / 12:03:33Z — distinct timestamps, the M236 stale-verdict guard), 15 containers not 16 (exactly the deleted router). **Clauses 1 + 4 hold; 2 (Playthroughs), 3 (migration map), 5 (KB-fidelity) remain.** `demo-1` is left UP + green for clause 2. (2)(3)(5) untouched; clause 3's iter-01 draft was lost with the old machine and restarts from nothing. rext pinned at tag fast-build-m257x-iter-13 (verified on origin). Re-scope trigger (two consecutive platform-invalidated attempts) stands at **occurrence 1 of 2**."
