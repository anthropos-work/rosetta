...
phase: "M257x iter loop, **131 iters** (+ 26 harden passes). **Gate 4 of 5**; clause 5 open at **`P = 29` / `N = 47`** (iter-131, corpus `60edbd84`). `P` is a **FLOOR over a suspicion-selected sample, never a corpus error rate**, and the **test-retest overlap with iter-119 is ~0** — two consecutive readings produced near-disjoint sets, so the rise 22 → 29 is not evidence the corpus worsened. ⚠ **OPEN: the 200-subagent cap left ONE independent adjudicator; the coordinator adjudicated 12 of 14 seats** — routed for re-adjudication; the single independent seat already moved `P` 30 → 29. Next repair: `FIX-M257x-iter131-infrastructure-hedge-stale` (11 sites + `CLAUDE.md`). **Detail — bands, composition, clusters — lives in `iter-131/progress.md` and the milestone `progress.md`. This field is a POINTER (`context.md` § state.md contract); do not grow it back.**"
