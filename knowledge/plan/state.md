...
**Active milestone:** **M6 "`dev-stack`: tooled local dev environment"** (section, `planned`) — bring the local dev env under the demo-stack pattern (multi-instance via the shared port-offset engine, **optional** Clerkenstein injection — default OFF), the heavy build still driven by rosetta's `/setup|start|update-platform` skills. Extract the shared `gen_override.py` (the M5-routed port-offset engine) here when dev-stack becomes its second consumer. Dir: [m6-dev-stack/](releases/01.10-show-floor/m6-dev-stack/).
**Next up:** **`/developer-kit:work-milestone M6`** on `release/01.10-show-floor`. Then M7 (seeding) → M8 (recipes) → `/developer-kit:close-release`.
**Last shipped:** **v1.0 "body double"** — 2026-06-03 (tag `v1.0`). · **Last closed milestone:** M5 — 2026-06-04.
**Paused:** _(none)_
