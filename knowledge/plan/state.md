...
active_milestone: "M257x — platform re-alignment (iterative) — IN PROGRESS, 116 iters + 25 harden passes closed (branch m257x/platform-realignment). Re-align BOTH rosetta (corpus) and rosetta-extensions (tooling) to platform @ origin HEAD. Gate 4 of 5; CLAUSE 5 — met only by a reading that returns zero — is the only open one, and iter-116 measured P = 37. **AWAITING A USER RE-SCOPE CONVERSATION**, per TOK-07's own pre-registered falsification. M257 PAUSED behind it after 3 iters."
last_closed: "M256 — 2026-07-30"
phase: "M257x ITER LOOP **PAUSED FOR RE-SCOPE**, local to the new Mac (D-v28-15). **Gate 4 of 5.** Clauses 1–4 hold; **CLAUSE 5 is the only open one**, met ONLY by a reading that returns zero — ruled FOUR times, never re-cut or narrowed, and NOT touched by any of the below. **iter-116 took `TOK-07` step 3, THE READ, against a rule sealed in `85f6f1c` before the first seat: `P = 37` / `N = 41`. The pre-registered `P >= 15` branch fired at more than DOUBLE the threshold, meeting `TOK-07`'s OWN falsification — repair-and-read is REFUTED as a path to clause 5 under this instrument, and the next move is a RE-SCOPE CONVERSATION WITH THE USER, not an eighth strategy revision. `TOK-08` is deliberately NOT authored.** The reading separates WHY, and that is the deliverable: **iter-115's repair HELD** (band #3 blind-on-predicate = 3 re-found of 24; `N`/`P` 1.50 -> 1.108), so **DENOMINATOR is FALSIFIED and VOLUME is the answer** — **25 of the 37 are standing pool no prior reading ever detected**, and **9 were induced by iter-115's own repair**, whose induction rate rose **5.6 % -> 22.0 %** as volume went **+48 -> +177** lines with the fences firing four times and still leaking. Series by predicate **22 -> 22 -> 24 -> 37**; by anchor **24 -> 33 -> 36 -> 41**. 7 of 14 bands held (6 of 8 mechanism, 0 of 3 magnitude); upheld rate **92.6 % raw / 92.6 % `wrong-tree`-separated** (`wrong-tree` 0; series 4 -> 1 -> 1 -> 0 -> 0). Two failures are findings: **intra-corpus mis-citation is now the LARGEST class (10 of 37)** — the corpus mis-cites ITSELF more often than it mis-describes the platform — and net-new band #12 shows **multi-pin blocks concentrate defects (6 of 41)**. 14/14 seats, 0 lost, committed verbatim before adjudication; 4/4 verdicts unedited; 54 booked -> 50 upheld. All 14 clones identical at close and open. **No repair taken inside the measuring pass**, including the guard-family FALSE RED disclosed at the open (`FIX-M257x-iter116-predicate-guard-takes-first-pin-in-block`). No point estimate of the pool is quoted — Chapman retired; floors only (>= 37 at `f581de09`). Detail: milestone `progress.md` + iter-116/adjudication.md. POINTER field per `context.md` § state.md contract; do not grow it back."
