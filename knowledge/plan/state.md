...
active_milestone: "M257x — platform re-alignment (iterative), **142 iters** + 29 harden passes closed (branch m257x/platform-realignment). Re-align BOTH rosetta (corpus) and rosetta-extensions (tooling) to platform @ origin HEAD. Gate 4 of 5; clause 5 the only one open, last measured **P = 29 / N = 47** (iter-131). M257 PAUSED behind it after 3 iters."
last_closed: "M256 — 2026-07-30"
phase: "M257x iter loop, **142 iters** (+ 29 harden passes; 11 tiks unhardened since pass 29). **Gate 4 of 5**; clause 5 open at **`P = 29` / `N = 47`** (iter-131, corpus `60edbd84`) — `P` is a **FLOOR over a suspicion-selected sample, never a corpus error rate**, and iters 132–141 took **no reading**, so the metric is UNMEASURED, not unmoved (`§9` iter-type refinement). Run 2 (iters 137–141) worked `TOK-08`'s mechanical classes and **retracted one of its own numbers**: iter-138's *"127 rotted pins / 57.2 %"* was audited at iter-139 on a pre-registered stratified sample and came back **0/12, Wilson95 [0.0, 24.3]** — withdrawn in place at all three publishing sites. The lesson that survives is `D-M257x-140-2`: **a class is censusable iff its subject carries its own HEAD** — receipts do (7/9 reproduce, 2 repaired), bare `:NN` pins do not. **Detail lives in the per-iter `progress.md` files and the milestone `progress.md`. This field is a POINTER (`context.md` § state.md contract); do not grow it back.**"
