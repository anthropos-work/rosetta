...
## Next-iter queue (RESUME HERE — run 4)
- **gate (g) full green** (iter-05 landed the root-cause capture fix; NOT yet green): (1) exercise the LIVE capture-path — re-pin billion to the fixed rext (tag `sound-check-m244-content-sweep-robustness`→`e74e563`), re-capture the directus surface + re-replay so `simulations_extraction` lands from the REAL capture (not the manual load); (2) verify the extraction sections (depth/adoption/sentiment) render behind "Explore Key Moments"; (3) add the plan-section-id alignment assertion + prove green.
- **the 2 voice cells → PRESENCE-ONLY** (decided): **Bunny keys are ABSENT on billion** (`BUNNY_RECORDING_CDN_TOKEN_KEY` + `BUNNY_RECORDING_PULL_ZONE_HOST` both absent from platform/.env + secrets + the jobsimulation container). Per the user decision, drop `hire-voice-fail` + `asmt-voice-pass-en` from the denominator: update `content-denominator.json` (49→47) with a recorded reason + drop the 2 cells from the manifest projection (fail-closed, no fabricated CTA). DEF-M240-01 stays presence-only (not landed).
- **intv-voice-pass/player residual** (SEPARATE from gate g): the terse-ack page fails "no acknowledgement" for the PASSED interview while the FAILED one passes — likely a sweep-assertion mismatch (a passed interview renders the result shell, not the ack regex). Investigate the passed-interview player page + fix the `player-interview` shape (or drop). After this + the voice-cell drop, gate (b) → its final green count.
