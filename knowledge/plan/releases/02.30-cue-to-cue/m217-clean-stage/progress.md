...
- [ ] **S1 — `corpus/ops/demo/demopatch-spec.md`** (the blind area — FIRST, before any code)
...
- [ ] **S2 — Port reap: bring-up + teardown** (scope 1)
...
- [ ] **S3 — Un-swallow the REFUSE reason** (scope 2)
...
- [ ] **S4 — `jobsimulation` exits(1)** (scope 4)
...
- [ ] **S5 — Re-pin the two `app` perf patches + the LOUD freshness preflight** (scope 3)
...
- [ ] **S6 — Prime the snapshot cache on `billion`** (scope 5)
...
- [ ] **S7 — Re-pin the drifted rext clones** (scope 6) — **AFTER S2's reap**
