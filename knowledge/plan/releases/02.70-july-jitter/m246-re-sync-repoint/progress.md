...
- [ ] **Seeder re-point** — rext `stack-seeding` writes `skillpath.skill_path_sessions → public.skill_path_sessions` in the **live** seeder code + tests (`cmd/stackseed/main.go:97`, `seeders/hero_activity.go:180`, `skillpath_sessions.go`, `content_nonsim.go`, `dna/data-dna.json` + the in-package test assertions). Surface **names** (`skillpath-sessions`) + the mirror `public.local_skill_path_sessions` left untouched.
