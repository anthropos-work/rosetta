...
- [ ] (note #2) Production-safe capture-source policy: cache-hit → read-replica refresh (fallback restore-from-backup) → `--allow-primary` last resort; bounded read-only session + catalog-first dry-run
