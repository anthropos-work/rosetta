...
- **iter-05 — the MANAGER vantage** (handler `MANAGER-M236-iter05-scoreboard`): **13 of the 15 non-landing pairs**. `No data` + `undefined undefined` on 11; **no header at all** on the 2 interview pairs (likely a separate route/tab — cf. M231's `flag_interview_manager_report`); 1 skill-path manager route times out. Evidence says **read-path, not seeding**: the mirror is 13/13 and the user row is correct (`Clara Romano`) — the player's own page renders her name fine. at `playbill-m235-hardened`, public-host default-on. Success = stack UP, fresh-green `autoverify.json`, cockpit serving a non-404 `content-manifest.json` with all 4 products. Handler `PHASE-C-M236-iter03-cold-bringup`. **Expect 30–45 min** — the host build cache was pruned, so this is a genuinely cold UI-tier rebuild.
