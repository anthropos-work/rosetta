...
- **iter-03 — Phase C: cold bring-up on `billion`** at `playbill-m235-hardened`, public-host default-on. Success = stack UP, fresh-green `autoverify.json`, cockpit serving a non-404 `content-manifest.json` with all 4 products. Handler `PHASE-C-M236-iter03-cold-bringup`. **Expect 30–45 min** — the host build cache was pruned, so this is a genuinely cold UI-tier rebuild.
