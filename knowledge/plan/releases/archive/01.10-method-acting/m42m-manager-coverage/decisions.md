...
| M42m-D6 | The manager gate is **reproducibly MET on a FRESH zero-manual `demo-up`** (the authoritative acceptance, iter-05) — and the M42e employee gate **holds** on the same fresh stack. The whole TOK-01 result (Studio demo-patch + FeedbackSeeder mirror + route reconcile + Sentinel reload + library replay + seeders) reproduces with **no manual step**. | iter-04 proved the gate on a *re-seeded live* demo-3; the gate language requires a *fresh build*. iter-05 tore demo-3 down `--purge` + removed the next-web image → a fresh `demo-up` auto-applied everything: demopatch (SERVED bundle 0×prod / 31×`localhost:39000`, clone git-clean after trap-revert), the FeedbackSeeder mirror (162/162 joinable, org=103), migrate + auto-set-dress (taxonomy + directus local-served + stories) + Sentinel `AuthorizationService/Reload` (no manual restart) + cockpit + autoverify GREEN. MANAGER sweep `reachable=70/120 (0,0,0,0) EXHAUSTED gateMet=true`; EMPLOYEE re-sweep `reachable=59/150 (0,0,0,0) EXHAUSTED gateMet=true` (no regression). One clone-cleanliness gap fixed in rext (R1b `.dockerignore` sweep, `method-acting-m42m-iter05`); the build-time `No space left on device` was an environmental Docker-VM disk ceiling (pruned + the same fresh `demo-up` resumed idempotently), not a tooling bug. Zero CANONICAL platform edits. See iter-05/decisions.md D1–D5. | 2026-06-26 |
