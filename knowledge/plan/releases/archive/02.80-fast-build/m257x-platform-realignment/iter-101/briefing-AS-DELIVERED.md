...
# ADDENDUM — readings #23 / #24 (M257x iter-101), 2026-08-06
...
|---|---|---|
| **platform** | `stack-demo/platform` | **`0c91421d`** — level with `origin/main`, re-verified by `git ls-remote` at this open (the table above says `0dab54df`; that is stale) |

**Every other checkout is IDENTICAL to the table above** and re-verified at this open: app `b948604f`,
...
ant-academy `9c3843cd`, rosetta-extensions authoring copy on `main`.

**Nested repos — `stack-demo/app/studio` and `stack-demo/cms/studio` are each their OWN git checkout at
`aeec036`**, not part of `app`'s tree. `git -C stack-demo/app grep <anything> HEAD -- studio/` returns
...
`corpus/services/**` + `corpus/architecture/**` — **40 files, 10,278 lines**, partitioned across 7 seats.
Two independent readings of the identical partition (#23 and #24). You are one blind seat: you do not know
which reading you are in, and you must not seek out any other seat's output.

## One thing that is TRUE and worth knowing

The corpus was repaired **at the immediately preceding commit** (8 citation anchors across 5 files). Repairs
induce defects — prior readings measured this directly. **Do not treat recently-edited prose as safer;
if anything treat it as more suspect.** Equally, do not book a claim merely because it looks new.
