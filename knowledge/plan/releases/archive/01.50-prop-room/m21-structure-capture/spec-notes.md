...
| — | **GATE automation clause** — *stacksnap* applies the captured structure | **DONE — GATE MET (iter-08):** iter-07 automated the SCHEMA (stages 3-4); iter-08 automated the SERVE rows (`directus_collections` registration + public-policy `directus_permissions`, behind the firewall structural-metadata admissibility class, M21-D13). `stacksnap` captures + auto-provisions schema + serve rows; a bootstrapped+provisioned stack serves anonymously with NO hand SQL. Full capture→apply Go path live-validated | the gate's "stacksnap applies" wording |
