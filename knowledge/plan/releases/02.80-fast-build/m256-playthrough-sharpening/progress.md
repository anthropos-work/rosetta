...
- iter-28 (tik, `closed-fixed-partial`): **the milestone's last UNKNOWN blocker is closed — the org-prepared
  onboarding trigger, found by READING the platform after a two-iter probe sweep could not find it.**
  `standard.UC2` had read *"the trigger is not yet identified"* since iter-08; iter-18 drove heroes across FOUR
  orgs and got the identical import form every time. It is one `useState` in the component **both apps mount**
  (`OnboardingUser.tsx:135`): `managerImport = Boolean(lastStep === OnboardingStep.Import && organizationName &&
  userStats)`. The host page always supplies the latter two, so the only missing input is `steps` — i.e.
  `public.user_params.onboarding`, **NULL for every seeded user**. *Which is exactly why probing could not find
  it: a four-org sweep was sampling a CONSTANT* (**D111**, the general rule: when a sweep returns the same
  answer for every vantage, the input is not one of the axes you are varying). `managerImport` swaps
  `<ImportStep>` for `<EnterpriseUser>` and relabels the forward control to *"Start"*. **Capability:**
  `Persona.Onboarding` (`""` | `"org_prepared"`) + `OnboardingParamsSeeder` writing ONE jsonb row whose last
  step is `import` — **not** `done`, which would complete onboarding and redirect. Closed enum (an unrecognised
  value falls back to the day-0 default, so the seat would be served the plain form and the Playthrough would
  fail *looking like a product regression*). 4 mutants RED. **And the insert alone SILENTLY DID NOTHING** —
  `public.user_params` is populated **row-per-user at user-insert time** (191 rows within 300 ms of the users
  COPY, all NULL, written by nothing in this fleet), so `ON CONFLICT (id) DO NOTHING` skipped it with no error
  and the seat kept getting the plain form; now insert-then-**HEAL**, and it **fails the seed** if a declaring
  hero's row cannot be reached, because that no-op presents as *"the product does not show the prepared
  summary"* (**D112**). The missing `audit.Record` was caught by the isolation guard on the first live run —
  two guards fired this iter and both were right. **Seat `pt-onboard-prepared` PROVEN LIVE**, and the
  discrimination is total: same run, `linkedinUrlInput` / `importFromLinkedInLabel` / `uploadButton` read
  **0 / 0 / 0** for her against **1 / 1 / 1** for `pt-free`, so every other seat in the world is a live
  contrast vantage. **And she cost a RED gate to place (D114):** first appended to Org A, `pt-workforce-funnel`
  went RED in **all three** runs — its iter-14 final asserts Pat Ellis's member-spotlight CARD carries her role,
  and one extra Org A member displaced Pat from the spotlight. *That is D107 one axis over* (there a role's
  occupancy perturbed a key-role card probabilistically; here an org's member SET perturbed a spotlight
  deterministically), and the rule is now explicit: **before appending a hero, check whether `seed-facts.ts`
  names her org** — Org B is the only pt org nothing anchors on, so she lives there. Re-proven on the SHIPPED
  seat (the org changed, and `organizationName` is one of `managerImport`'s three inputs): import form 0/0/0,
  the relabelled **Start** control present, and **her own name** on the summary. **The Playthrough is ROUTED,
  not attempted (D113):** the UC's flow continues *"confirm the
  pre-filled role, refine the suggested skills"*, and whether the relabelled *"Start"* control completes
  onboarding or advances to the Role step is **unmeasured** — the iter's own escalation condition named that
  stop, and writing the spec anyway would assert a multi-step journey nobody drove, which is the iter-22 failure
  (a spec that *could not have passed*) and the iter-27 failure (a read-back that read the seed). The residual
  is **one probe run wide**. **`196 passed` ×3 cold, rc 0, 0 flake** after the move; controls 26 of 28. rext `71a2d9f` + the close commit. — see [`iter-28/progress.md`](iter-28/progress.md)


- iter-29 (tik, `closed-fixed`): **onboarding 3 of 5 → 4 of 5 — only the org-less UC remains — and the
  milestone's signature defect was caught in this iter's OWN work by the standing mutant iter-27 routed for
  exactly that purpose.** iter-28 routed this Playthrough as *"one probe run wide"*, and the probe answered:
  the prepared variant's relabelled **`Start`** CONFIRMS and ADVANCES — `user_params.onboarding` goes
  `[{import}]` → `[{import},{role}]` and the flow lands on the SKILLS screen (`Change Role` + her declared
  role's **real taxonomy skills**), so the curated *"confirm the pre-filled role, refine the suggested skills"*
  is that one click and its outcome. `pt-onboarding-org-prepared` GREEN at 2.1 s. **THEN S1c — iter-27's
  standing Q1 mutant (delete the action and the intermediates, leave only the read-back) — PASSED.** The
  read-back asserted *"the prepared summary is gone"* after a fresh `goto('/onboarding')`; the reasoning was
  CORRECT (once a `role` step exists `managerImport` cannot be true) and the assertion was still **worthless**,
  because `toHaveCount(0)` immediately after a navigation is satisfied by a page that has **not hydrated yet**.
  **Removed, not weakened** (**D115**) — an absence assertion a mid-hydration page satisfies is worse than none,
  because it reads as proof; the honest repair needs a POSITIVE locator on the ROLE screen, which nothing has
  driven, so the persistence half is routed. *This is the THIRD variation on one theme — iter-12 (a dead page
  satisfies every absence), iter-22 (`rows > 0` satisfied by "No roles match your filters"), and now **TIME** as
  the confounder: the page is fine, it just is not there yet. An absence assertion needs a companion that proves
  WHEN it was read, not only WHERE.* What the UC lands on is mutation-proven both ways: **S1** (delete the click)
  RED at "Change Role", **S2** (remove `onboarding: org_prepared` + reseed) RED at liveness — the summary never
  renders and the import form returns, so the SEED is load-bearing. The pre-state doubles as a cross-vantage
  control: the import form EVERY other seat in the world is served reads **0** for her.
  **`197 passed` ×3 consecutive cold reset-to-seed, rc 0 each, 0 flake**; `ptreport` **29/31, 0 failing, 0
  unimplementable, 2 TODO**; controls **27 of 29**, MUTATES **11**; `--policy-check` green; 16 containers Up / 0
  exited; fixture restored byte-identically. — see [`iter-29/progress.md`](iter-29/progress.md)


- iter-30 (tik, `closed-fixed`): **the exit gate's LAST gap is priced from MEASUREMENT instead of estimate —
  and the blocker carried in prose since iter-08 was wrong in both directions.** `individual.UC1`'s routed
  blocker read *"needs a seeder change (a member-less user + a roster seat)"*. Nobody had ever tested the
  load-bearing half. Measured the cheap way — **delete** a seeded hero's membership on `demo-2` (backed up,
  restored by `--reset`) rather than write a seeder to find out — and **an org-less user REACHES THE APP**: she
  logs in, `/onboarding` **serves the flow** (`#linkedinUrl` present, 5 buttons, `bodyLen` 392, **0 page
  errors**), and `/home` renders (14 buttons). So the UC is **landable**, the re-scope trigger moves further
  away rather than closer, and the Phase-0b audit's F5 (*"no pre-onboarding state exists and none can be
  declared"*) is refuted on its **last** foothold. **And the cost was understated ~5×:** `memberships` carries
  **12 incoming FKs across 8 tables**, the un-cascaded delete fails loudly with
  `violates foreign key constraint "membership_skills_memberships_membership"`, and one hero's live dependents
  are `membership_skills` **7** · `membership_languages` **2** · `membership_tags` **1** ·
  `organization_target_roles` **1**. So it is **≥ 5 seeders**, not one — *and the FK list is only the entry
  fee*: the real work is the quiet tail, the persona/profile/activity seeders writing rows that carry an
  `organization_id` for a user with no membership, which do not FK-break and are simply wrong invisibly (the
  iter-28 D112 silence class). **Deliberately nothing built** — the iter's plan forbade starting the seeder
  change, because its size was the thing being measured. **And a pattern closed the session (D117): THREE of
  the five iters found a routed blocker mis-stated** — iter-26's *"needs a stage-0 seat"* (it could not be
  declared at all), iter-28's *"trigger not identified"* (one `useState`; the four-org sweep was sampling a
  constant), iter-30's *"a seeder change"* (landable, and ≥ 5). Each was written in good faith and each read as
  a measurement by the next iter. **The rule: a routed blocker must carry the measurement that produced it, or
  be marked `(estimate, unmeasured)`.** — see [`iter-30/progress.md`](iter-30/progress.md)


- iter-31 (tik, `closed-fixed`): **the routed persistence repair was IMPOSSIBLE — and finding out why exposed a
  platform defect that only the seat iter-28 minted can reach.** iter-29 routed it as *"one measurement: a
  POSITIVE locator on the ROLE screen a reload lands on, because the component opens on `lastStep || Import` and
  `lastStep` is now `role`"*. **There is no such screen.** A fresh navigation re-serves the **pre-state**
  byte-identically — progress 0, `[Skip] [Start]`, no "Change Role", no skills — with the `role` step verified in
  the DB each time, across **six fresh navigations in three browser sessions**. The mechanism is **one array read
  from both ends**: `useGetOnboardingStatus.tsx:25-27` sorts `onboarding.steps` **NEWEST-FIRST** and
  `OnboardingUser.tsx:130-132` then takes `steps[steps.length-1]` as `lastStep` — **the OLDEST step ever taken** —
  so `managerImport` is true again and the initial step is `Import` again, forever; confirm your role, reload, and
  you are back at step one with the work persisted and invisible. The host page reads the SAME array at index 0
  and is correct, which is why completion redirects properly and nothing else ever looked wrong. **It is
  unreachable for a NULL or single-element array — every one of the 191 seeded users** — so only a MULTI-step
  array exposes it, and the only multi-step user in existence is `pt-onboard-prepared`: *the seed capability built
  to reach a surface turned out to be the only way to see a defect on it* (iter-11's shape, one domain over).
  A **second** defect on the same journey: the flow **dead-ends** at *"Add more skills"* — *"We're having trouble
  loading your skills at the moment"*, `Next` **inert**, clicked five times — so the `/onboarding` → `/home`
  completion read-back is unreachable too, and a spec written for it *could not have passed* (iter-22's mode).
  Both routed to the **platform** at milestone level, never only in a spec comment (the iter-23 rule); zero
  platform edits. **What shipped instead (D119): the measurement plus the LABEL** — a positive, hydration-proof
  assertion on the screen the reload really lands on, stating in the docstring, the failure message and the
  manifest that it proves *the confirmation left her flow intact* and is **NOT** the write's read-back. RED three
  ways (**S3** route-anchor · **S4** identity-anchor · **S5** a planted `done` step, which really does eject her —
  and S5 fires at the FIRST liveness assert rather than the new line, because a `done` state cannot be introduced
  mid-flow from outside, *stated not glossed*); **S1** re-confirmed RED. The page object's own docstring —
  claiming `changeRoleControl()` *"on a FRESH navigation is the server-side read-back"*, written in good faith at
  iter-29 — was **false and is corrected in place**, because a docstring is read as evidence by the next iter.
  **D118 extends iter-30's D117 one class over: a source read is a HYPOTHESIS, and citing a `file:line` does not
  make it an observation** — this blocker was not a mis-priced cost but a **mis-stated behaviour**, and the
  citation beside it is exactly what made it read as measured. **AND THE TWO DURABLE HALVES.** (a) The rule
  iter-12 wrote in prose, iter-22 re-broke and iter-29 broke again **in a brand-new spec one hour after writing
  the prose** is now machine-checked for the whole suite (`liveness-before-absence-fence.unit.spec.ts`): after a
  navigation, an absence assertion needs a liveness witness first — the same defect in three costumes, **dead**
  (iter-07 `bodyLen 24`) · **empty** (iter-22, the placeholder row that IS a row) · **not there YET** (iter-29,
  pre-hydration), the third being why the first two fixes did not prevent it, since *an absence assertion needs a
  companion proving WHEN it was read, not only WHERE*. **Sized before adopting** (iter-15 D74): 29 files · 62
  navigation sites · 184 witnesses · 37 absence assertions · **0 violations** — already true everywhere, **zero
  edits**, so it buys the *next* spec; fail-closed on three floors; proven discriminating by re-introducing
  S1c's exact shape. (b) **Every uncovered use case now carries a WRITTEN VERDICT the four-state map renders
  (D121).** `unimplemented` was one state wearing two meanings: every TODO reported *"declared use case, no
  Playthrough yet (build-reference gap)"* — **false** for `standard.UC1`, which is a measured refusal, not an
  effort gap — so the artifact tooling and reviewers read stated the wrong position about the very use case the
  gate is being closed around. Now a closed `disposition` (`will-not-build` | `not-yet-built`) + `measured_by` +
  `rationale` + `handler`, fenced **bidirectionally** (a TODO without one fails; a **live** UC that still carries
  one fails — a stale verdict has no expiry) and **against vacuity** (an 80-char floor, nine placeholder
  spellings, `measured_by` required — D117 mechanised; the enum closed with **no fallback member**, because this
  milestone twice shipped a seed enum whose unrecognised value fell back to a permissive default and in both
  cases would have produced a Playthrough that looked like a *product regression*; a handler REQUIRED for a gap
  and FORBIDDEN for a refusal, the asymmetry being what stops the two blurring into *"TODO with a paragraph
  attached"*). Four states and glyphs **unchanged**. **Two existing tests failed and that was the point** — both
  encoded *"a bare TODO is legitimate"*, the position this revokes; renamed and updated, never relaxed. **Six
  mutants RED against the SHIPPED manifest** (iter-16's lesson: a fence proven only on fixtures is proven against
  itself) + 20 Go tests incl. the rationale floor from both sides, a shipped-corpus check that fails closed on an
  empty load, and the closed-set message asserted deterministic over 20 runs. `standard.UC1` **deliberately not
  landed and no route to landing it sought** (D122, `D104` upheld — the 13th iter). **`199 passed` ×3 consecutive
  cold reset-to-seed, rc 0 each, 0 flake**; `ptreport` **29/31, 0 failing, 0 unimplementable, 2 TODO both
  verdicted**; controls **27 of 29**, MUTATES **11**; `--policy-check` rc 0 (`live=18 expected=18`); 16
  containers Up / 0 exited; fixture sha-verified `99e2f315` after each reset; all four Python suites green
  (999/287/171/266). rext `27b9710` + `992aee3`. — see [`iter-31/progress.md`](iter-31/progress.md)


- iter-32 (tik, `closed-fixed`): **the LAST un-homed use case in the M201 curated corpus is LIVE — and the
  work was not the FK list, exactly as iter-30 predicted.** `onboarding.individual.UC1` had no milestone
  anywhere for five releases and is the one use case where this milestone's pre-flight audit had a kernel of
  truth: `UsersSeeder` wrote a membership for EVERY seeded user unconditionally, so an org-less actor could
  not be expressed. The capability is `Persona.OrgMembership: none` — her `public.users` row and her
  Clerkenstein identity exist and **nothing org-scoped does**. **It has TWO halves and iter-30's measurement
  only exercised one (D123):** deleting a membership row left Clerk still carrying the org, so the IDENTITY
  half is net-new and is load-bearing — the host page mounts `<OnboardingIndividual>` vs `<OnboardingUser>`
  off `useGetClerkOrganization`, so a DB-only org-less hero would have been served the ENTERPRISE flow and
  the Playthrough would have proven the ordinary journey. **`verified: 0` is REQUIRED of her, not permitted
  (D124)**, and that inversion is what keeps the change small: a verified skill's fan-out is org-scoped, so a
  "verified org-less hero" writes rows tying her to an org she is not in; requiring 0 makes the whole
  persona/profile/activity chain skip her **because she declares nothing** instead of because fifteen seeders
  each learned a special case. **BOTH halves of iter-30's warning then arrived inside one iter (D125).** The
  LOUD half announced itself on the first reseed — `SuccessionSeeder` FKs each member's first population
  session, which she no longer has, and the seed failed **by name**
  (`interview_extraction_results_sessions_session`), friendly exactly as predicted. The QUIET half was found
  by **sweeping the live DB for her uuid across every uuid column** in `public` + `jobsimulation`: **2
  `jobsimulation.sessions` rows carrying an `organization_id`**, plus 4 activity events, 2 skill-path
  sessions, 4 personal assignments and 3 bookmarks that made a "day-0" user look like a returning one — none
  of which breaks anything. Post-fix the sweep reads **10 tables, ALL user-scoped, 0 with `organization_id`
  or a membership FK, 0 casbin grants**; Org B seeds **19 memberships against `size: 20`**, the one slot she
  consumes without becoming a member. Because the quiet half has **no static signature**, the two halves get
  **two different guarantees and the fence SAYS SO**: `orgless_writers_fence_test.go` enumerates the
  `membershipUUID(prefix, i)` call sites — *those ARE the FK surface* — and requires each to consult the
  predicate (its own self-test caught an error in its fixture first), while the org-scoped tail is covered by
  the sweep. *A fence that implied it covered both would be more dangerous than none, because the uncovered
  half is the half that fails silently.* **TWO PRE-EXISTING FENCES REFUSED THE SEAT BEFORE IT SHIPPED, and
  neither refusal came from review (D126):** the M224 curated-pool fence rejected `Product Designer` (no
  curated family → the taxonomy's alphabetical junk head), then the M219-R8 ladder fence rejected the
  replacement because her default **65**-skill claimed tail would have drawn the `operations` family dry and
  shipped a *silently thin* profile. **The fence's own suggested remedy — grow the allow-list — was the wrong
  fix**: an org-less day-0 user should claim nothing, and now does. What she is served is a **third
  component**, not a variation ( *"Your journey is about to start…"* · her role to confirm · `[Change Role]
  [Done]` · **0 textboxes**), so the enterprise import form every member gets is her in-line cross-vantage
  control; and her read-back EXISTS where her sibling's does not, because the host page reads `steps[0]` —
  the newest step — which is the one reading of that array the platform gets right (iter-31's defect is the
  other end of it). **The test never navigates to `/home` by hand**: her home renders BEFORE she onboards, so
  the only observation the write can satisfy is that the app itself takes her OFF the flow. **5 mutants RED**
  — N1 (no click), **N4 (iter-27's standing Q1: action + intermediates deleted, read-back only)**, N3
  (another hero's role), **N2 (drop the seed declaration + reseed → she comes up an ordinary member)**, and
  **V7** (a verdict re-added to the now-LIVE UC → `ptvalidate` rc 1: iter-31's bidirectional fence fired live
  for the first time, and **landing this UC is what forced the verdict's removal**). **The first mutant pass
  was THROWN AWAY and that is the lesson (D127):** against a world the green drive had consumed, N1 went red
  at the wrong line and **N4 PASSED** — a false pass indistinguishable from a weak assertion, caused entirely
  by state. *For an irreversible write the protocol is reset → mutate → run, every time.* **`200 passed` ×3
  consecutive cold reset-to-seed, rc 0 each, 0 flake** — and **an earlier batch of three carried ONE failure,
  reported not re-rolled (D128)**: `pt-workforce-succession`, a Playthrough this iter did not touch in an org
  it did not touch (Org A measured unchanged: 40 memberships, occupancy 1), diagnosed as a recurrence of
  `PLATFORM-M256-keyrole-nondeterminism` that **extends** iter-26's record — iter-26 measured 5/5 present at
  occupancy 1, this absence was AT occupancy 1 — with 6/6 passing on immediate re-measurement and **no
  timeout bumped** (iter-26 retracted exactly that "fix"). `ptreport` **30/31 passing, 0 failing, 0
  unimplementable, 1 TODO**; controls **28 of 30**, MUTATES **12**; `--policy-check` rc 0; datadna closure
  PASS; 16 containers Up / 0 exited; fixture sha-verified `99e2f315` after all 10 resets. rext `f46476a`.
  **The exit gate is MET.** — see [`iter-32/progress.md`](iter-32/progress.md)
...
| ~~`ONBOARD-M256-prepared-playthrough`~~ | **DONE (iter-29).** `pt-onboarding-org-prepared` is live: the prepared summary is served INSTEAD of the import form (a cross-vantage control against every other seat), and confirming advances into the skills step with her role's real taxonomy skills. S1 + S2 RED; **S1c PASSED and the assertion it exposed was REMOVED** (D115). | closed iter-29 |
| ~~`ONBOARD-M256-prepared-persistence`~~ | **CLOSED as REFUTED (iter-31), and the refutation is worth more than the fix would have been.** The routed measurement — *"drive a fresh `/onboarding` after the role step is confirmed and record a POSITIVE locator on the **Role** screen"* — assumed a screen that **does not exist**: the reload re-serves the pre-state summary byte-identically (six fresh navigations, three sessions, the `role` step verified in the DB each time), because `useGetOnboardingStatus.tsx:25-27` sorts the steps **newest-first** and `OnboardingUser.tsx:130-132` then reads `steps[length-1]` as `lastStep`, i.e. **the OLDEST step**. So the flow can never resume, the persistence half is **not assertable through the UI at all**, and the cause is a platform defect rather than a harness weakness → re-routed as `PLATFORM-M256-onboarding-step-not-resumed`. What shipped instead is the measurement plus a LABEL (D119): a positive, hydration-proof assertion stating it proves *the confirmation left her flow intact* and is **not** the write's read-back, RED three ways. The absence-only assertion was **not** restored. **D118:** a source read is a hypothesis — citing a `file:line` does not make it an observation. | closed iter-31 |
| `PLATFORM-M256-onboarding-step-not-resumed` | **NET-NEW (iter-31), routes to the PLATFORM — two defects on one journey.** (1) **The org-prepared onboarding flow cannot resume.** The confirmation persists server-side and a reload puts the member back on step one, progress 0, with no trace of it — *one array, two consumers, opposite ends*: the fetch hook sorts `onboarding.steps` NEWEST-FIRST (`useGetOnboardingStatus.tsx:25-27`) and the shared component takes `steps[steps.length-1]` as `lastStep` (`OnboardingUser.tsx:130-132`), the OLDEST step; the host page reads index 0 and is correct, which is why completion redirects properly and nothing looked wrong. **Unreachable for a NULL or single-element array — all 191 seeded users** — so only the multi-step seat `pt-onboard-prepared` can see it. (2) **The flow dead-ends on a demo:** *"Add more skills"* renders *"We're having trouble loading your skills at the moment"* and its `Next` is **inert** (5 clicks, identical screen, progress stuck at 100), so the `/onboarding` → `/home` completion read-back is unreachable from this seat — recorded so nobody writes a spec expecting it. Full record at MILESTONE level in [`decisions.md`](decisions.md). **Zero platform edits; the platform tree was read only.** | **captured iter-31 → platform** || ~~`ONBOARD-M256-orgless-seat`~~ | **DONE (iter-32) — `pt-onboarding-individual` is LIVE; the verdict's removal was FORCED by iter-31's bidirectional fence (mutant V7 RED), not remembered. The capability is `Persona.OrgMembership: none` with TWO halves (DB + Clerk identity — iter-30's delete only exercised the first), `verified: 0` REQUIRED, 11 seeders skipping her slot behind a source-scan fence, and the org-scoped tail proven empty by a live uuid sweep (10 tables, all user-scoped, 0 org-scoped, 0 grants). Two pre-existing fences refused the seat first.** Original pricing: **PRICED FROM MEASUREMENT (iter-30) — replaces `ONBOARD-M256-import-path`'s `individual.UC1` line.** LANDABLE: an org-less user (no membership row at all) logs in, is served `/onboarding`, and reaches `/home` — measured by DELETING a hero's membership on demo-2, not by writing a seeder first. COST: **≥ 5 seeders**, not the "a seeder change" carried since iter-08. `UsersSeeder` must skip the membership row + its casbin grants, and the four membership-keyed seeders must skip her (`membership_skills` 7 rows · `membership_languages` 2 · `membership_tags` 1 · `organization_target_roles` 1, for ONE hero); the other four of `memberships`' 12 incoming FKs write nothing for a hero today but must be AUDITED, not assumed. **The FK list is the entry fee, not the work** — FK breakage fails loudly and names its consumer; what will not fail is the persona/profile/activity seeders writing `organization_id`-bearing rows for a user with no membership (the iter-28 D112 silence class). Then the Playthrough is small: all four of its surfaces are already measured live. | ~~**iter-32**~~ **DONE** |
| ~~`ONBOARD-M256-import-path`~~ | **DISCHARGED BY VERDICT (iter-31), which is the sanctioned outcome — not by landing.** `standard.UC1` now carries a machine-checked `verdict` block in the manifest: `disposition: will-not-build`, `measured_by: iter-18 (six live probe passes) + D104`, **no handler** (a refusal with an assignee is a contradiction), and the rationale states all three facts — the journey IS drivable, its only advancing path **scrapes a live public third-party profile** on a site that blocks automation (so its RED would read as an Anthropos regression), and the deterministic CV route is blocked by a **product defect we do not own** (200 POST on a valid PDF *and* a docx while the forward control never enables, 100 s+). `ptreport` renders that verdict in place of the generic *"build-reference gap"* line, so the four-state map finally states the position. **iter-18's refusal was upheld, for the 13th iter, and no route to landing it was sought** (D122). | closed iter-31 || `PT-M256-standing-mutant-Q1` | **NET-NEW (iter-27).** Run *"delete the action and see whether anything fails"* against EVERY mutating Playthrough on this world, not just new ones. Q1 found a green-without-the-write in a spec that had already been written and reviewed once, and the check costs one edit plus one run. On a seeded world the outcome is usually already present, so this is the cheapest detector of the milestone's signature defect. | a later tik of M256 || ~~`VERDICT-M256-remaining-uncovered`~~ | **DONE (iter-09).** [`coverage-verdicts.md`](coverage-verdicts.md): all **28** curated UCs verdicted, **0 `unimplementable`**. Clause 3's verdict half is COMPLETE. | closed iter-09 |
