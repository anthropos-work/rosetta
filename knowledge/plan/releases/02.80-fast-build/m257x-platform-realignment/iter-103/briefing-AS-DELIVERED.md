...
# ADDENDUM — readings #25 / #26 (M257x iter-103), 2026-08-06
...
|---|---|---|
| **platform** | `stack-demo/platform` | **`0c91421d`** — level with `origin/main`, re-verified by `git ls-remote` at this open |
| **app** | `stack-demo/app` | **`ad9f3c49`** — the checkout is now **level with `origin/main`** (the table above says `b948604f`; that is stale, though still a reachable ancestor, so a claim *pinned* at `b948604` is still graded there) |
...
For these six the **checkout is behind `origin/main`** (storage `9f8cb532`, messenger `e9421c68`,
...
| authoring copy | `.agentspace/rosetta-extensions` | `944fc4a2` on `main` |
...
`corpus/services/**` + `corpus/architecture/**` — **40 files, 10,646 lines**, partitioned across 7 seats.
Two independent readings of the identical partition (#25 and #26). You are one blind seat: you do not know
which reading you are in, and you must not seek out any other seat's output.

## One thing that is TRUE and worth knowing

The corpus was **repaired at the immediately preceding commit, and heavily** — 52 audit anchors paid out to
**98 sites across the tree**, and the in-scope corpus grew **10,278 → 10,646 lines**. Repairs induce
defects; prior readings measured this directly and the rate has held for five consecutive cycles.
**Do not treat recently-edited prose as safer; if anything treat it as more suspect.** Equally, do not book
a claim merely because it looks new, and do not book a *retraction* merely because it contradicts the thing
it retracts — a retraction saying "X was wrong, the truth is Y" is correct prose, not a self-contradiction.
