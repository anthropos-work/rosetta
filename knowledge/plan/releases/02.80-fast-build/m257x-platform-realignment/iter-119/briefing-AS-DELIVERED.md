...
# ADDENDUM — readings #31 / #32 (M257x iter-119), 2026-08-07
...
both sides printed in `ground-truth.md`, and `diff` returned empty; `git log --follow` shows exactly one
...
**Every clone above is at the SAME sha it was at the previous THREE readings.** No repo has advanced since
iter-103's reading. That is a property of this reading, not a reason to relax: **the previous readings
proved that a claim can be false against a sha that never moved** — platform-drift was ~33 % of iter-109's
residual and ~38 % of iter-116's, over a subject in which literally nothing changed.
...
| authoring copy | `.agentspace/rosetta-extensions` | **`43049308`** on `main` |
...
`corpus/services/**` + `corpus/architecture/**` — **40 files, 10,871 lines**, partitioned across 7 seats.
Two independent readings of the identical partition (#31 and #32). You are one blind seat: you do not know
which reading you are in, and you must not seek out any other seat's output.

## What has and has not changed in the corpus since it was last read

**The in-scope corpus is 5 changed lines away from the last reading, and its line count is IDENTICAL
(10,871).** The two intervening iters censused *citations* — they enumerated every intra-corpus citation
(1,520) and every platform-source citation candidate (1,070) and repaired **8 broken `#anchor`
fragments**, of which **2 sites land in this scope** (`architecture_overview.md`,
`platform-migration-status.md`). **No prose was rewritten and no proposition was changed.**

**Do not read that as "the corpus is now clean."** It means the opposite of a large repair: almost nothing
in your files has been touched since a reading that found a substantial residual. **Treat every claim as
unaudited.** Equally, do not book a claim merely because it looks old, and do not book a *retraction*
...
## One thing that is FALSE and that you should NOT be misled by

A guard in the tooling (`platform_predicate_guard`, check G10) reports `corpus/services/sentinel.md:5` as
declaring the wrong compose-service count. **That guard verdict is itself wrong** — it grades the sentence
at the *first* platform ref appearing in the paragraph rather than at the ref the sentence itself names,
and the sentence is true at the ref it names. Graded and routed at the previous reading's open, still open
and deliberately unfixed (no repair is taken inside a measuring pass). It is stated here so that a seat
which independently notices it books what it measured, not what a guard said.
