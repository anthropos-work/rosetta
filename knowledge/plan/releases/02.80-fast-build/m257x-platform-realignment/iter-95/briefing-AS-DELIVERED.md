...
# ADDENDUM — ground truth RE-DERIVED at this reading's open (2026-08-05)

**Everything above this line is the frozen instrument, byte-identical (sha256 `3858ec53…`). Nothing in
it has been edited.** This addendum supplies only the two things the instrument requires to be
re-derived at every open: the current clone refs, and your assignment.

## ⚠️ The sha table in the frozen briefing above is STALE. This table supersedes it for refs ONLY.

The briefing's ground-truth table was captured at iter-76. **`platform` has since moved
`0dab54df` → `0c91421d`** (three platform moves have landed; the most recent, `838d907`/`0c91421`,
deleted the `storage`, `messenger` and `customerio-sync` compose services outright and removed
`storage` + `messenger` from `repos.yml`). Every other clone's **checkout is unchanged** from the
briefing. Use the table below.

| clone | checkout (on disk) | `origin/main` | behind |
|---|---|---|---|
...
**This sheet is authoritative for refs.** If you grade a claim against a ref not on this sheet, say
which ref you used and why.
...
1. **Grade at the ref the claim names — UNLESS the sentence asserts currency**, in which case no
   neighbouring pin rescues it. (A seat given only a checkout sha cannot see that the checkout is
   stale; the `origin/main` column exists so you can.)
2. **A corpus citation into `rosetta-extensions` grades against the AUTHORING copy**
   (`.agentspace/rosetta-extensions`, `6130bfd8`) unless the citing block pins a ref. The
   `stack-demo/` clone is a different, older clone of the same repo.

## A note you should read before booking a `platform` anchor

`platform` moved since the frozen briefing was written. A compose/`repos.yml` line anchor that
resolved at `0dab54df` may now land on a different construct at `0c91421d`. **Per the briefing's own
rule, a claim that names no ref is graded against the checkout — which is now `0c91421d`.** Open the
file and read the surroundings; do not assume the anchor.

## HARD BARS — restated, and they are absolute

- **You MUST NOT read anything under `knowledge/plan/**`.** That is the answer key of prior audits.
  Do not `grep` it, `ls` it, or open it. Your report does NOT go there.
- **You MUST NOT read anything under `.agentspace/scratch/`.**
- **You MUST NOT read any other seat's output.** Do not list or open any directory other than your
  own output directory. Other seats are reading in parallel right now.
- **Read-only.** Zero edits to any file except your single output report.
