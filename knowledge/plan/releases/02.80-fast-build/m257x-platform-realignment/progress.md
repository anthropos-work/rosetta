...
| `HOST-M257x-toolchain` **PARTLY CLOSED (iter-04); MOVED AGAIN (iter-06)** | No `pytest` on the system `python3` (only the ephemeral `/tmp/rextvenv`), no `gh`/`psql`/`tailscale`. **Closed at iter-06:** the Playwright/npm e2e tests now pass (`stack-verify` 210/210, was 207+3F). **Opened at iter-06:** `go test ./...` in `stack-seeding` cannot resolve the private `ai v1.40.1` module without `GOPRIVATE=github.com/anthropos-work/*` **plus** a `url."git@github.com:".insteadOf "https://github.com/"` rewrite — the same **Trap E** class as iter-04's `apply-authn.sh`, this time in the developer's own Go env rather than in rext code. | iter-04 |
...
| `REPOINT-M257x-cms-similarity-writes` **NEW (iter-06)** | `stack-snapshot/simembeddings` replays `cms.similarities` + `similarity_{categories,features,skills}`; cms-in-app moved all four to `public` and the `cms` schema is an empty shell. Already firing on demo-1 as `sim-embeddings replay skipped (rc=4)`. It is the **last** transitional entry — i.e. the last thing between here and a claimable **gate clause 4**. | next tik |
