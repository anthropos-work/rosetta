...
| `HOST-M257x-stack-demo` | ~~No `stack-demo/` workspace~~ — **clone set landed iter-03**. Residual, much smaller: finish `ensure-clones.sh`'s trailing phases (idempotent re-run), provision secrets values-blind, then attempt the first `demo-up`. | iter-04 |
