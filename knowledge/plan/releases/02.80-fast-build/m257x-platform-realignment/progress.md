...
| item | why | target |
|---|---|---|
| **`SURVEY-M257x-iter143-bare-orphan-bucket`** *(measured 2026-08-08, iter-142 close; the next iter's starting point)* | `TOK-08`'s pre-registered class list has **two** classes. Class 1 (intra-corpus citation resolution) is fenced and GREEN. **Class 2 (platform-source citation resolution) is fenced at 59.1 % reach** — `anchor_construct_guard` resolves **861 of a 1,456 denominator**, 0 findings, and the largest single unresolved head is **`(bare)` at 384**, the orphan/ambiguous continuation pins. **The guard's own comment says *"this is the bucket the ports are in"*, and that was MEASURED at iter-142's close and is only marginally true: of all 621 bare-anchor matches corpus-wide, just 38 carry a known platform/demo port value.** So the orphan bucket is overwhelmingly REAL citations with an undetermined head — which is exactly what `FIX-M257x-iter138-anchor-rot-fence`'s re-specification means by *head resolution first*. ⚠️ **Do not ship a head-inference heuristic without a hand-read precision audit first** — an unaudited mechanical predicate over this exact population is what iter-138 published and iter-139 retracted at 0-for-12. | iter-143 |
