...
**Six seats across four independent file sets converged on one thing:** the corpus says
`infrastructure` "has never been in any clone set" and that `cms`'s production state is "NOT MEASURABLE
— do not assert either way", **while the corpus elsewhere cites 28 reads of that repo at a pinned sha.**
...
| sites still saying it was never in a clone set / unmeasurable | **11** |
| `ls -d stack-*/infrastructure` today | absent — **it is not in the clone set NOW** |

**Both halves are true and that is the whole point.** iter-123 cloned it, read it, and the corpus
records the result at `org-repos.md:102` in a heading that could not be plainer:
...
**So the hedge is the stale side, and the correction reached one file.** This is the milestone's own core
...
**`P` = 30 distinct false predicates**, adjudicated, in-scope, upheld.
**`N` = 47 distinct anchors.** `N`/`P` = **1.57**.

### The predicates

| # | predicate | anchors | class |
|---|---|---|---|
| P1 | `infrastructure` has never been in any clone set / is unreadable | cms.md:16,:61,:218 · backend.md:51,:86-87 · jobsimulation.md:54 · skiller.md:26 · external_services.md:175 · storage.md:175 | self-contradiction |
| P2 | cms's production ECS state is NOT MEASURABLE; do not assert either way | cms.md:14,:17 · backend.md:13 · jobsimulation.md:12 · platform-migration-status.md:310 | self-contradiction |
