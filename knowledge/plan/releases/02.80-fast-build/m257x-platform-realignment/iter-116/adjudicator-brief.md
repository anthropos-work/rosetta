# iter-116 adjudicator brief — held fixed at the iter-95/97/99/101/103/109 shape
...
| rosetta-extensions (**authoring**) | `.agentspace/rosetta-extensions` | `1dc1eb82` on `main` | — |

**Every one of the 14 platform clones is at the same sha it was at the previous TWO readings.** That is
context, not licence: the previous reading measured **~33 % platform-drift over a subject in which nothing
...
1. **A claim is settled at the ref the claim itself NAMES** (`platform-alignment.md` §5 rule 33). A pin is a
   **date**, not an excuse: if the claim is true at its named ref it is **TRUE**, however stale. A pin's
   ...
   names its own ref** — a guard in our own tooling got exactly this wrong at this reading's open and
...
- `.agentspace/rosetta-extensions` — the **authoring copy**, `1dc1eb82` on `main`. Where the next tag is
