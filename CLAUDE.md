...
**Shared Libraries** — **do not read this list as `app`'s dependency set. It is not one, and as of
2026-08-15 `app` HAS no org-module dependency set at all.** Measured at `app` **`4bccda085`** (v2.3.2):
`grep anthropos-work/ app/go.mod` returns **one line — the `module` declaration itself. ZERO requires.**
The taxonomy-v2 program finished the fold `ai` began (`e72f18199`, *"fold taxonomy in — app has no
first-party module left"*), and **`colony` went with it**; `colony/authn` is now in-tree at
`app/internal/authn`. ⚠️ **This block read *"requires **five** org-private modules"* at `3eaadae6` and
**seven** at `b948604f` — three answers in one release cycle, so re-measure before citing it.** It is
not a documentation detail: Clerkenstein's demo disarm cloned colony and added a `go.mod` replace, so
with no require it failed FATAL and no demo could log in (fixed rext `v2.9.2` by swapping the in-tree
package). The repos still exist and are still owned; `app` simply no longer imports them. Full picture:
...
- ~~taxonomy~~: **FOLDED INTO `app` at `e72f18199`** (2026-08-14) — *"app has no first-party module left"*. It was a **node-id library** (`NodeID` type + generation/validation), never a dataset. The skill/job-role DATA lives in `app`'s `public` schema (**42,908 physical public rows** as of 2026-08-26; was ≥42,790 at the 2026-06-29 measurement), and the **governed canon** — 3,562 skills / 706 roles in the checked-in artifact at `app/taxonomy-canon/` — is **LIVE in production since (at latest) 2026-08-26**: the serving set is `organization_id IS NULL AND deleted = false` (3,562 / 709), the legacy rows are soft-deleted in place. See [`taxonomy-canon.md`](corpus/architecture/taxonomy-canon.md) §8
...
- Studio-Desk (**Next.js 16 + React 19, Node ≥24 — ONE process, ONE port**): Design tool for creating simulation blueprints (repo: `studio-desk`). ⚠️ **This row read *"TypeScript/Vite/Express"* until 2026-08-17.** The migration **merged to `main` 2026-08-23** (PR #123, merge commit `2ddf2ee3`, 874 commits) and deleted `src/`, `vite.config.ts` and every `*.html` entry point; the API is now route handlers in the same Next process, client env is `NEXT_PUBLIC_*` (**inlined at build time**), and the image is `output: 'standalone'` → **119.6 MB** (was 1.35 GB). **Ports: container `9000 + N*OFFSET`, native dev server `9200`** — `9100` was the Vite port and is dead. **`platform/docker-compose.yml` has NOT been updated**: it still passes `VITE_*` build args the new Dockerfile does not declare, which docker discards silently, producing an image that builds, starts, reports **healthy** and 500s every page. Rosetta injects the right args from an rext override instead, and **since 2026-08-23 it does so unconditionally** — a plain `dev-stack up N --profile all` is correct. (`--studio-src <tree>` now means only *build from a different tree*, e.g. a feature worktree; it was the switch that turned the right args ON until the migration landed on main, which made the default path the broken one.) See [`corpus/services/studio-desk.md`](corpus/services/studio-desk.md)
...
> **⚠️ A `dev-N` / `demo-N` stack starts ONE MORE container than this table shows — `lodge`.** The table
> describes the **platform's own compose**, and it stays exactly right for `make up` (the main dev stack,
> `dev-0`). Since v2.10 M272 the rext bring-ups additionally emit a **lodge** service into the stack's
> generated override — `hyper-studio`'s deployable — so `dev-stack up N` / `demo-up N` bring up **five**
> containers on the default profile, not four. It declares **no `profiles:` key**, which (exactly like
> `postgresql` and `redis`) puts it in *every* selection.
>
> Lodge serves two listeners: the job **wire** (`8080 + N·10000`) and the **operator panel**, a real
...
- **`corpus/services/README.md`: the enumerated index of all 28 service docs** — start here rather than guessing a filename. Grouped into core backend (Tier 1), frontends & gateway, cross-cutting subsystems / `app`-owned domains (AI-readiness, Course Builder, AI Labs + credits, Ask/Talk-to-Data, Academy backend, hiring, Clerk, Clerkenstein, **Lodge** — `hyper-studio`'s deployable, net-new in every stack at v2.10 M272), and archived/merged redirects (`skiller`, `skillpath`, `chronos`, `intelligence`)
